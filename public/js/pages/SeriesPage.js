...
class SeriesPage {
    ...
    async handleCatalogLanguageFacetsInvalidated(event) {
        if (event?.detail?.type !== 'series') return false;
        ...
        await this.populateLanguageFacets({ force: true });
        if (refreshFilteredView && this.app?.currentPage === 'series') {
            this.onFiltersChanged();
        }
        return true;
    }
    ...
    shouldShowRails() {
        ...
        return !this._isTvMode() && this.isCloudPagedMode() &&
            !!window.GenreRails && (!this.sourceSelect?.value || !!this.selectedCloudSourceId()) && !this.hasActiveFilters();
    }
    ...
    maybeRevalidate() {
        ...
        if (this.seriesList.length && this.catalogCacheKey()) {
    ...
    isBrokenItem(item) {
        ...
        return (health?.isUnavailableEntry
            ...
            (health?.isUnavailable
                ? health.isUnavailable(item.sourceId, 'series', item.series_id)
                : health?.isBroken(item.sourceId, 'series', item.series_id));
    }
