...
// BEGIN GENERATED SOURCE INPUT POLICY
...
const SOURCE_ATTEMPT_PATH_SHAPES = Object.freeze([
...
function normalizeSourceInputHostname(hostname) {
...
function parseSourceInputUrl(raw) {
...
function sourceInputProblem(raw) {
  const value = String(raw ?? "").trim();
...
function classifySourceAttemptPath(raw) {
...
function normalizedSourceAttemptDomain(hostname) {
...
return Object.freeze({ SOURCE_ATTEMPT_PATH_SHAPES, normalizeSourceInputHostname, parseSourceInputUrl, sourceInputProblem, classifySourceAttemptPath, normalizedSourceAttemptDomain });
})();
// END GENERATED SOURCE INPUT POLICY
...
class SourceManager {
    ...
    sourceFormatSwitcher(type) {
        ...
        return `
          <section class="source-format-switcher" aria-labelledby="source-format-switcher-title">
            ...
            <button class="setup-compatibility-help" type="button" data-source-help-open>
    ...
    bindSourceFormatSwitcher(type, { focusSelected = false, drafts = {} } = {}) {
        const modal = document.getElementById('modal');
        ...
        const activate = (tab) => {
            ...
            // Drafts live only in this modal's closure, never in storage/telemetry.
            const currentDraft = {};
            ['url', 'name', 'username', 'password'].forEach((field) => {
                currentDraft[field] = document.getElementById(`source-${field}`)?.value || '';
            });
            currentDraft.inputPathShape = document.getElementById('source-url')?.dataset.sourceInputPathShape || '';
            this.showAddModal(nextType, { focusFormat: true, drafts: { ...drafts, [type]: currentDraft } });
        };
    ...
    sourceProviderHelp() {
        return `<section class="setup-assistance-panel source-provider-help" data-source-help-panel aria-labelledby="source-help-title" hidden>
          <button class="setup-back-button" type="button" data-source-help-back><span aria-hidden="true">${Icons.chevronRight}</span><norva-i18n data-i18n="ui_web_7524c5f6d71d">Back to connection formats</norva-i18n></button>
          <h2 id="source-help-title" tabindex="-1" data-i18n="ui_web_ec8a17582163">An app login cannot be imported directly</h2>
          ...
          <div class="setup-provider-request">
            ...
            <div class="setup-provider-message" data-source-provider-message>
              <p data-i18n="ui_web_251db7feb14c">Hello, could you please send me one of these access formats?</p>
              <ol><li data-i18n="ui_web_90b277f27c7c">My complete M3U/M3U8 playlist URL</li><li data-i18n="ui_web_6d771c44295a">My Xtream Codes details: server URL (including the port), username and password</li></ol>
              <p data-i18n="ui_web_8569efc4ed5a">Thank you.</p>
            </div>
          </div>
          <button class="btn btn-primary setup-copy-message" type="button" data-source-help-copy data-i18n="ui_web_457efe538b25">Copy message</button>
          <p class="setup-copy-feedback" data-source-help-feedback role="status" aria-live="polite" aria-atomic="true"></p>
        </section>`;
    }

    bindSourceProviderHelp(modal) {
        const main = modal.querySelector('[data-source-connection-view]');
        const panel = modal.querySelector('[data-source-help-panel]');
        const trigger = modal.querySelector('[data-source-help-open]');
        const footer = modal.querySelector('#modal-footer');
        if (!main || !panel || !trigger || !footer) return;
        let previousFooterHidden = footer.hidden;
        const back = () => {
            panel.hidden = true;
            main.hidden = false;
            footer.hidden = previousFooterHidden;
            delete modal.__sourceHelpBack;
            trigger.focus({ preventScroll: true });
        };
        trigger.addEventListener('click', () => {
            previousFooterHidden = footer.hidden;
            main.hidden = true;
            panel.hidden = false;
            footer.hidden = true;
            modal.__sourceHelpBack = back;
            panel.querySelector('h2')?.focus({ preventScroll: true });
            modal.querySelector('#modal-body').scrollTop = 0;
        });
        panel.querySelector('[data-source-help-back]').addEventListener('click', back);
        const copy = panel.querySelector('[data-source-help-copy]');
        copy.addEventListener('click', async () => {
            if (copy.disabled) return;
            copy.disabled = true;
            const status = panel.querySelector('[data-source-help-feedback]');
            try {
                // Only this static help text is copied, never a form value.
                await navigator.clipboard.writeText(panel.querySelector('[data-source-provider-message]').innerText);
                status.textContent = globalThis.NorvaI18n?.t('ui_web_5e19d709b999', { defaultValue: 'Message copied to clipboard.' }) ?? 'Message copied to clipboard.';
            } catch (_) {
                status.textContent = globalThis.NorvaI18n?.t('ui_web_885e4c5495fb', { defaultValue: 'Could not copy automatically. Select the message above to copy it.' }) ?? 'Could not copy automatically. Select the message above to copy it.';
            } finally {
                copy.disabled = false;
                if (copy.isConnected && !panel.hidden && document.activeElement === document.body) {
    ...
    showAddModal(type, { focusFormat = false, drafts = {} } = {}) {
        const modal = document.getElementById('modal');
        ...
        body.innerHTML = formatSwitcher
            ? `<div class="source-add-shell"><div data-source-connection-view>${formatSwitcher}<div class="source-format-panel" id="source-modal-connection-panel" role="tabpanel" aria-labelledby="source-format-${type}">${sourceForm}</div></div>${this.sourceProviderHelp()}</div>`
            : sourceForm;
    ...
    `;
        ...
        if (window.NorvaModal?.installHygiene) NorvaModal.installHygiene(modal, {
            onClose: () => modal.__sourceHelpBack ? modal.__sourceHelpBack() : closeModal()
        });
        ...
        for (const field of ['url', 'name', 'username', 'password']) {
            const control = document.getElementById(`source-${field}`);
            if (control && drafts[type]?.[field]) control.value = drafts[type][field];
        }
        if (SourceInputPolicy.SOURCE_ATTEMPT_PATH_SHAPES.includes(drafts[type]?.inputPathShape)) {
            document.getElementById('source-url').dataset.sourceInputPathShape = drafts[type].inputPathShape;
        }
        this.bindSourceFormatSwitcher(type, { focusSelected: focusFormat, drafts });
        ...
        this.bindSourceProviderHelp(modal);
    }
    ...
    `;
      ...
      <div class="form-group">
        ...
        <input type="text" id="source-url" name="provider-server-url" class="form-input"
               ...
               aria-describedby="source-url-parse-hint source-url-error" data-source-existing="${isExisting}">
        ${type === 'xtream' ? '<p class="hint" id="source-url-parse-hint" data-i18n="ui_web_bfb93439a4d0">If you paste a full Xtream link, Norva will fill the login fields automatically.</p>' : '<p class="hint" id="source-url-parse-hint" data-i18n="ui_web_1612432ddb45">Enter a complete http or https address supplied by your provider.</p>'}
        <p class="form-error source-input-error" id="source-url-error" role="alert" aria-atomic="true" hidden></p>
    ...
    renderSourceInputFeedback(type) {
        const input = document.getElementById('source-url');
        const error = document.getElementById('source-url-error');
        const hint = document.getElementById('source-url-parse-hint');
        if (!input) return null;
        const feedback = this.sourceInputFeedback(input.value, type);
        // An encrypted saved connection may expose only a display hint. A
        // metadata-only edit is not a request to validate/replace that secret.
        if (input.dataset.sourceExisting === 'true' && !/^https?:\/\//i.test(input.value)
            && !document.getElementById('source-username')?.value.trim()
            && !document.getElementById('source-password')?.value.trim()) return null;
        input.setAttribute('aria-invalid', String(feedback.state === 'invalid'));
        if (hint) hint.hidden = feedback.state === 'invalid';
        if (error) {
            error.textContent = feedback.state === 'invalid' ? feedback.message : '';
            error.hidden = feedback.state !== 'invalid';
        }
        if (hint && feedback.state !== 'invalid' && feedback.message) hint.textContent = feedback.message;
        return feedback;
    }
    ...
    bindSourceForm(type) {
        ...
        if (!urlInput) return accessWizard;
        urlInput.addEventListener('input', () => {
            delete urlInput.dataset.sourceInputPathShape;
            urlInput.removeAttribute('aria-invalid');
            const inlineError = document.getElementById('source-url-error');
            if (inlineError) { inlineError.hidden = true; inlineError.textContent = ''; }
        });
        urlInput.addEventListener('blur', () => this.renderSourceInputFeedback(type));
        urlInput.addEventListener('change', () => this.renderSourceInputFeedback(type));
        urlInput.addEventListener('paste', () => setTimeout(() => {
            if (urlInput.isConnected) this.renderSourceInputFeedback(type);
        }, 0));
        ...
        const applyParsedLink = (force = false) => {
            // Never rewrite an email/name into a plausible provider host.
            if (!urlInput.isConnected || this.renderSourceInputFeedback(type)?.state === 'invalid') return;
        ...
        if (sourceOnboarding && connectionStep && connectionNext && accessWizard?.fieldset) {
            ...
            const formatSwitcher = sourceOnboarding.closest('.modal')?.querySelector('.source-format-switcher');
            ...
            const showSourceConnectionStep = ({ focus = true } = {}) => {
                ...
                if (formatSwitcher) formatSwitcher.hidden = false;
            ...
            const showSourceAccessStep = () => {
                ...
                if (feedback.state === 'invalid') {
                    ...
                    connectionError?.scrollIntoView?.({ block: 'nearest', behavior: 'auto' });
                    return;
                }
                ...
                try {
                    ...
                    if (connectionError) {
                        connectionError.textContent = this.sourceFormErrorMessage(error);
                        connectionError.hidden = false;
                    }
                ...
                if (formatSwitcher) formatSwitcher.hidden = true;
    ...
    parseXtreamLink(raw) {
        const value = String(raw || '').trim();
        const url = SourceInputPolicy.parseSourceInputUrl(value);
        if (!url) return null;
    ...
    sourceInputPathShape(raw) {
        return SourceInputPolicy.classifySourceAttemptPath(raw);
    }

    sourceInputProblemMessage(problem, type = 'm3u') {
        if (problem === 'email_or_login') {
            return globalThis.NorvaI18n?.t('ui_web_source_email_not_url', { defaultValue: 'This is an email or app login, not a provider URL. Ask your provider for a complete M3U link or Xtream server details.' })
                ?? 'This is an email or app login, not a provider URL. Ask your provider for a complete M3U link or Xtream server details.';
        }
        if (problem === 'name_not_url') {
            return type === 'xtream'
        ...
        return globalThis.NorvaI18n?.t('ui_web_1612432ddb45', { defaultValue: 'Enter a complete http or https address supplied by your provider.' })
            ?? 'Enter a complete http or https address supplied by your provider.';
    }
    ...
    sourceInputFeedback(raw, type = 'm3u') {
        ...
        const problem = SourceInputPolicy.sourceInputProblem(value) || (pathShape === 'web_page' ? 'web_page' : null);
        if (problem) {
            return {
                state: 'invalid', pathShape, problem,
                message: this.sourceInputProblemMessage(problem, sourceType)
            };
        }
    ...
    sourceAttemptRootDomain(rawHostname) {
        return SourceInputPolicy.normalizedSourceAttemptDomain(rawHostname);
    }

    async sourceAttemptDiagnostic(input = {}) {
        const rawUrl = String(input.url || '').trim();
        if (!rawUrl) return null;
        const parsedUrl = SourceInputPolicy.parseSourceInputUrl(rawUrl);
    ...
    buildSourceConnection(input = {}) {
        ...
        const replacesConnection = !existing || (['auto', 'xtream'].includes(requestedType)
            ? Boolean(username || password) : /^https?:\/\//i.test(rawUrl));
        if (rawUrl && replacesConnection) {
            const feedback = this.sourceInputFeedback(rawUrl, requestedType);
            if (feedback.state === 'invalid') {
                throw Object.assign(new Error('Invalid provider address.'), {
                    code: 'INVALID_SOURCE_ADDRESS', inputProblem: feedback.problem, sourceType: requestedType
                });
            }
        }
        ...
        if (credentialsProvided && ['m3u', 'epg'].includes(type) && rawUrl
            && !/^https?:\/\//i.test(rawUrl) && SourceInputPolicy.parseSourceInputUrl(rawUrl)) {
            // Add a scheme only AFTER the original input passed strict syntax
            // validation; the Edge API requires an absolute HTTP(S) URL.
            url = `http://${rawUrl}`;
        }
    ...
    sourceFormErrorMessage(error) {
        ...
        const allowed = new Map([
            ...
            ['Enter the complete server URL, username and password to replace the saved login.', 'ui_web_7c425ffe7b0c'],
            ['Provider URL, username and password are required.', 'ui_web_4b98db7e65d5']
        ]);
        return allowed.has(message)
            ? (globalThis.NorvaI18n?.t(allowed.get(message), { defaultValue: message }) ?? message)
    ...
    hostFromUrl(raw) {
        return SourceInputPolicy.parseSourceInputUrl(raw)?.hostname || '';
    }
    ...
    async saveNewSource(type) {
        ...
        try {
            ...
            if (err?.code === 'INVALID_SOURCE_ADDRESS' || !urlInput?.value.trim()) {
                const inlineError = document.getElementById('source-url-error');
                if (inlineError) { inlineError.textContent = safeMessage; inlineError.hidden = false; }
                const inputHint = document.getElementById('source-url-parse-hint');
                if (inputHint) inputHint.hidden = true;
                urlInput?.setAttribute('aria-invalid', 'true');
                urlInput?.focus({ preventScroll: true });
                // The keyboard can shrink the scroll area above the fixed
                // footer. Keep the complete recovery instruction visible.
                inlineError?.scrollIntoView?.({ block: 'nearest', behavior: 'auto' });
                // The field already explains the problem. Do not stack a toast
                // (or fallback alert dialog) over it and steal recovery focus.
