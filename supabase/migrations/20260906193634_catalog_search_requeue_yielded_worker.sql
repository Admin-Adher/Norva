...
-- Cooperative yield retains state=processing after expiring the lease. A row
-- lock, expired lease and empty inflight set are sufficient to requeue safely.
-- Never reset or advance the durable cursor, owner snapshot or lease sequence.
...
begin
  ...
  if v_checkpoint.state not in ('pending','processing')
