-- Admin dashboard — "sync incomplète" detector: VOD-only, not live-only providers.
--
-- The detector flagged any source with media_items but no cloud_title_variants. But a LIVE-TV-only
-- provider (e.g. AtlasPro: 10 843 live channels, 0 VOD) legitimately has media_items and 0 variants
-- (variants/titles are built from VOD movies/series only) — so it was a false positive. Only flag a
-- source that has VOD items (item_type in movie/series) yet produced no variants (a real stalled
-- finalize). Only the two `sources_incomplete` / `incomplete` predicates change.
