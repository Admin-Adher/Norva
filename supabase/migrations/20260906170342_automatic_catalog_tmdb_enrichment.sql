...
-- Before the existing genre-column trigger; all other import/tenant guards stay.
create trigger trg_cloud_titles_enrichment_preserve before update on public.cloud_titles
for each row execute function public.norva_preserve_catalog_tmdb_enrichment();
create trigger trg_candidate_titles_enrichment_preserve before insert or update on public.cloud_source_catalog_generation_candidate_titles
for each row execute function public.norva_preserve_catalog_tmdb_enrichment();

-- Reuse the installed endpoint, secret lookup and timeout. No environment URLs
-- or credentials are copied into the migration, and no global cursor is reset.
do $schedule$
declare search_job record; owner_command text;
begin
  select * into search_job from cron.job where jobname='norva-enrich-search-match';
  if not found or position('/cron/search-match?' in search_job.command)=0 then
    raise exception 'installed TMDB search job is required';
  end if;
  owner_command := regexp_replace(search_job.command,'/cron/search-match\?[^'' ]*','/cron/catalog-owner-maintenance');
  perform cron.schedule('norva-catalog-owner-maintenance','* * * * *',owner_command);
  perform cron.alter_job(search_job.jobid,schedule=>'*/2 * * * *',
    command=>regexp_replace(search_job.command,'/cron/search-match\?[^'' ]*','/cron/search-match?limit=120&conc=4'));
  perform cron.alter_job(jobid,schedule=>'1-59/5 * * * *',
    command=>regexp_replace(command,'/cron/revalidate\?[^'' ]*','/cron/revalidate?limit=120&conc=4'))
    from cron.job where jobname='norva-enrich-revalidate';
end
$schedule$;
commit;
