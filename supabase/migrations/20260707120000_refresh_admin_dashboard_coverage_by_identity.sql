-- Formalize refresh_admin_dashboard() with the coverage-by-(user+identity) grouping.
--
-- REPRODUCIBILITY-GAP CLOSURE (2026-07-07). The coverage (`cov`) block of
-- refresh_admin_dashboard() was changed BY HAND in prod during the 2026-07-07 session
-- (group admin audio-coverage by provider IDENTITY instead of by raw source) and was
-- never captured in a migration. This file captures the CURRENT prod definition verbatim
-- so a fresh environment (e.g. the Hetzner AX42 self-host) reproduces it exactly. It is a
-- no-op against the current prod DB (the body already matches). No behavior change.
--
-- The relevant change vs the pre-session version is in the `cov` CTE: it now joins
-- catalog_provider_identities (by config_hint->>'providerKey') + provider_identities and
-- groups by coalesce(pi.id::text, s.id::text) so multiple sources sharing one panel
-- identity collapse into a single coverage row.
