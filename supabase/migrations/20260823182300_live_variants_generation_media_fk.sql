...
begin
  ...
  if not exists (
    ...
    where constraint_state.conrelid = 'public.cloud_live_variants'::regclass
      and constraint_state.conname = 'cloud_live_variants_generation_media_item_fk'
  ) then
    alter table public.cloud_live_variants
      add constraint cloud_live_variants_generation_media_item_fk
  ...
  perform public.norva_assert_catalog_generation_contract_constraint(
    'cloud_live_variants_generation_media_item_fk', false
  );
end
$install$;
