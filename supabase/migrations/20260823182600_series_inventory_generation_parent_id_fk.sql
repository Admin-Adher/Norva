...
begin
  ...
  if not exists (
    ...
    where constraint_state.conrelid = 'public.catalog_series_inventory_state'::regclass
      and constraint_state.conname = 'catalog_series_inventory_gen_parent_id_fk'
  ) then
    alter table public.catalog_series_inventory_state
      add constraint catalog_series_inventory_gen_parent_id_fk
  ...
  perform public.norva_assert_catalog_generation_contract_constraint(
    'catalog_series_inventory_gen_parent_id_fk', false
  );
end
$install$;
