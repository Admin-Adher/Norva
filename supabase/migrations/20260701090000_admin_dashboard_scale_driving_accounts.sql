-- Admin dashboard — scale to thousands of users by bounding the OPS sections to the enrichment
-- DRIVING accounts (a fixed small set) instead of scanning every user's catalogue.
--
-- Enrichment crons run ONLY for the driving accounts (super8k / apdxes / airo); every other user
-- inherits enrichment via the cross-user cache (keyed by provider identity). So per-panel coverage
-- and per-source scan detail are only meaningful for the drivers. Previously the refresh scanned
-- ALL cloud_titles (O(total titles)) and ran 4 count subqueries PER source over ALL sources
-- (O(sources × titles)) — fine for 3 users / 557k titles, fatal at thousands of users / tens of
-- millions of titles. Now:
--   • Enrichment coverage → driving accounts only (bounded).
--   • Providers → driving panels + PROBLEM sources (errored / VOD-with-no-variants), capped 300.
--   • Overview KPIs stay global aggregates (headline totals) — cheap counts.
-- Add a driving account by inserting a row here; the crons' userIds are the source of truth.

create table if not exists public.admin_enrichment_accounts (
  user_id  uuid primary key,
  label    text,
  added_at timestamptz not null default now()
);
alter table public.admin_enrichment_accounts enable row level security;  -- read only via gated RPCs
insert into public.admin_enrichment_accounts(user_id, label) values
  ('c5be5ac4-3700-4a25-9509-8eaf7771fdb6','super8k'),
  ('0b971271-9fa1-4547-8dc6-ab64dcbb9d33','apdxes/jeremy'),
  ('7bdab1df-80e6-46f9-bcdf-84b6595819a8','airo')
