-- Slice 4 (anti-ban, honnêteté dashboard) — expose resolved_24h par panel×type.
--
-- L'incident Ninja a montré que la ligne films MENT : 7 410 « sondé_24h » mais 0 résolu (le chemin
-- film stampe même en échec → débit « sain » trompeur). Le seul signal fiable de « provider mort/
-- banni » est : sondé_24h > 0 ET résolu_24h = 0. On ajoute `resolved_24h` au blob coverage ; le
-- front s'en sert pour l'alarme « provider muet » + le garde-fou d'ETA (« ≫ 1 an »).
--
-- Re-émission verbatim de refresh_admin_dashboard() (20260702140000) + 1 colonne dans le bloc cov.
