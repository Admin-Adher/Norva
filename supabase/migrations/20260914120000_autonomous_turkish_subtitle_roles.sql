-- Autonomous exact Turkish subtitle-role corrections; no generic aliases.
...
declare
  ...
  category_key text; category_match text[]; declared_french_multi boolean;
...
-- BEGIN GENERATED AUDITED CATEGORY LABELS
  category := coalesce('{"FR I BOX OFFICE & ULTRA 4K":"FR | BOX OFFICE & ULTRA 4K","أفلام فرنسية":"FRENCH MOVIES","Séries ARABES":"ARABIC SERIES","مسلسلات عربية | SERIES ARABES":"ARABIC SERIES","افلام عربية | FILMS ARABES":"ARABIC MOVIES","أفلام عربية قديمة | ANCIENS FILMS ARABES":"ARABIC MOVIES","مسلسلات تركية مدبلجة عربي | Turkish Series Arabic Dub":"ARABIC DUB","Séries DUB-AR":"ARABIC DUB","4K | أفلام عربية":"ARABIC MOVIES","يعرض الآن عربي":"ARABIC SERIES","SÉRIES TURK":"TURKISH SERIES"}'::jsonb ->> btrim(regexp_replace(category,'\s+',' ','g')),category);
-- END GENERATED AUDITED CATEGORY LABELS
  category_match := regexp_match(category,'^\s*([A-Z]{2,3})\s+I\s+(.+)$');
  if public.catalog_provider_language_alias(lower(category_match[1])) is not null then
    category := category_match[1] || ' | ' || category_match[2];
  end if;
  category_match := regexp_match(category,'^\s*MOVIES\s+([A-Z]{2,3})(\s+(?:[0-9]{4}(?:-[0-9]{4})?|CLASSICS\s*&\s*RARITIES))?\s*$');
  if public.catalog_provider_language_alias(lower(category_match[1])) is not null then
    category := category_match[1] || ' | MOVIES' || coalesce(category_match[2],'');
  end if;
  category := regexp_replace(category,'^\s*EX-YU(?=\s*[|:]|\s*$)','EXYU','i');
  nl_hindi_category := category ~* '^\s*NL\s*\|\s*HINDI\s*$';
  category_key := btrim(regexp_replace(category,'\s+',' ','g'));
  -- These five English shelves independently declare the audio. A final
  -- Arabic subtitled marker must not hide a conflicting (French) annotation;
  -- remove only that suffix from the local value before all existing guards.
  if category_key in ('MOVIES - ENGLISH 2024',
    ...
    'مسلسلات إنجليزية 2025 | English Series 2025') then
    raw := regexp_replace(raw,'\s+مترجم\s*$','');
  end if;
  -- Complete, audited category + bare-prefix pairs assign dub destinations.
  -- Keep independent suffixes for conflict checking and every file observation
  -- above this parser in the existing effective-language functions.
  -- Audited market-prefixed shelves. Preserve independent suffix conflicts.
  -- IN alone has no audio meaning; EN/KOREAN do not assert a second track.
  if category_key in ('LA - PELÍCULAS','LA - INFANTILES','LA - ANIME')
    and raw ~ '^\s*LA(?=\s*[-–—]\s+|\s+[-–—]\s*|\s*[|:])' then
    raw := regexp_replace(raw,'^\s*LA','ES'); category := 'SPANISH MOVIES';
  elsif category_key='IN - EN HINDI'
    and raw ~ '^\s*IN-EN(?=\s*[-–—]\s+|\s+[-–—]\s*|\s*[|:])' then
    raw := regexp_replace(raw,'^\s*IN-EN','HI'); category := 'HINDI MOVIES';
  elsif category_key='IN - KOREAN HINDI DABBLING'
    and raw ~ '^\s*IN-(?:EN|KD)(?=\s*[-–—]\s+|\s+[-–—]\s*|\s*[|:])' then
    raw := regexp_replace(raw,'^\s*IN-(?:EN|KD)','HI'); category := 'HINDI DUB';
  elsif category_key='افلام فرنسية مترجمة'
    and raw ~ '^\s*FR-AR(?=\s*[-–—]\s+|\s+[-–—]\s*|\s*[|:])' then
    raw := regexp_replace(raw,'^\s*FR-AR','FR'); category := 'FRENCH MOVIES';
  end if;
  -- Full provider shelf + prefix evidence; no country or generic AR/TR alias.
  if category_key='مسلسلات تركية مترجمة'
    and raw ~ '^\s*AR-TR-S(?=\s*[-–—]\s+|\s+[-–—]\s*|\s*[|:])' then
    raw := regexp_replace(raw,'^\s*AR-TR-S','TR'); category := 'TURKISH SERIES';
  elsif category_key in ('Séries TURQUES SUB-AR','مسلسلات تركية مترجمة عربي | Turkish Series Arabic Sub') then
    category := 'TURKISH SERIES';
    -- Remove only the redundant terminal [SUB]; preserve preceding conflicts.
    raw := regexp_replace(raw,'\s+\[SUB\]\s*$','','i');
  end if;
  -- Autonomous audit: exact Turkish movie/current-series subtitle shelves.
  if category_key='افلام تركية مترجمة'
    and raw ~ '^\s*AR-TR(?:-S)?(?=\s*[-–—]\s+|\s+[-–—]\s*|\s*[|:])' then
    raw := regexp_replace(raw,'^\s*AR-TR(?:-S)?','TR'); category := 'TURKISH MOVIES';
  elsif category_key='يعرض الآن تركي مترجم'
    and raw ~ '^\s*AR-(?:TR-S|SUBS)(?=\s*[-–—]\s+|\s+[-–—]\s*|\s*[|:])' then
    raw := regexp_replace(raw,'^\s*AR-(?:TR-S|SUBS)','TR'); category := 'TURKISH SERIES';
  end if;
  if category_key in ('افلام تركية مدبلجة','مسلسلات تركية مدبلجة','يعرض الآن تركي مدبلج')
    and raw ~ '^\s*AR-TR-D(?=\s*[-–—]\s+|\s+[-–—]\s*|\s*[|:])' then
    raw := regexp_replace(raw,'^\s*AR-TR-D','AR-D'); category := 'ARABIC DUB';
  elsif category_key='مسلسلات اجنبية قصيرة مدبلجة'
    and raw ~ '^\s*AR-EN-S-D(?=\s*[-–—]\s+|\s+[-–—]\s*|\s*[|:])' then
    raw := regexp_replace(raw,'^\s*AR-EN-S-D','AR-D'); category := 'ARABIC DUB';
  elsif category_key='HINDI DUBBED (KOREAN)' and raw ~ '^\s*IN(?:-?KD)?(?=\s*[-–—]\s+|\s+[-–—]\s*|\s*[|:])' then
    category := 'HINDI DUB';
  elsif category_key='INDIA EN DUBBED' and raw ~ '^\s*IN(?=\s*[-–—]\s+|\s+[-–—]\s*|\s*[|:])' then
    category := 'EN | DUB';
  elsif category_key in ('TURKSIH SERIES (SUB EN)','EN - ITALIAN SUB ENG')
    and raw ~ '^\s*EN(?=\s*[-–—]\s+|\s+[-–—]\s*|\s*[|:])'
    and raw !~ '^\s*EN\s*[-–—|:]\s*EN\s*[-–—|:]' then
    raw := regexp_replace(raw,'^\s*EN',case when category_key like 'TURKSIH%' then 'TR' else 'IT' end);
    category := case when category_key like 'TURKSIH%' then 'TURKISH SERIES' else 'ITALIAN SERIES' end;
  elsif category_key='ASIA MOVIES (MULTI-SUBS)' and raw ~* '\(Cantonese ver\.\)\s*(?:(?:19|20)[0-9]{2})?\s*$' then
    category := ''; raw := regexp_replace(raw,'\(Cantonese ver\.\)','(Cantonese version)','i');
  elsif category_key='AR★ أفلام كرتون للأطفال' and raw ~ 'مدبلج'
    and raw !~ 'مترجم|ترجمة|زیرنویس|زیرنویس‌دار' then
    category := 'ARABIC DUB';
  end if;
  -- MULTI permits only the explicitly declared FR on these French shelves;
  -- it does not assert another audio language, a track count, or verification.
  declared_french_multi := category_key ~* '^FR:\s*(?:FILMS\s*-\s*[^|:]+|Documentaires)\s*$'
    or category_key='FRANÇAIS';
  ...
  suffix := coalesce((regexp_match(without_year,'(?:^|\s)([[:alpha:]ऀ-ൿ]{4,30})\s+\((?:Dubbed|Dub)\)\s*$','i'))[1],
    (regexp_match(without_year,'[[(]([[:alnum:]ऀ-ൿ ./+-]{2,40})[\])]\s*$'))[1],
  ...
  audio_blocked := raw ~* '\m(?:vost\w*|subtitles?|subbed)\M'
    -- A title such as المترجم (The Translator) is not a subtitle marker.
    or raw ~ '(?:^|[^[:alpha:]ऀ-ൿ])(?:مترجم|ترجمة|زیرنویس|زیرنویس‌دار)(?=$|[^[:alpha:]ऀ-ൿ])';
  for idx in 1..3 loop
    part := case idx
    ...
    if annotated then
      ...
      part := regexp_replace(part,'^(?:19|20)[0-9]{2}[-.]([[:alpha:]ऀ-ൿ]{4,30})$','\1');
    ...
    if has_multi and not declared_french_multi then audio_blocked := true; end if;
    foreach token in array tokens loop
