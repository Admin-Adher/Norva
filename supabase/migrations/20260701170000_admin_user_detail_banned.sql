-- Add `banned` (suspension state) to the admin_user_detail profile so the fiche can badge a
-- suspended account and toggle Suspendre/Réactiver correctly. Only the profile select gains a column;
-- the driver/normal source+coverage branches are byte-identical to 20260701150000.
create or replace function public.admin_user_detail(p_user_id uuid)
returns jsonb language plpgsql volatile security definer set search_path = public as $$
declare v_user jsonb; v_email text; v_driver boolean; v_sources jsonb; v_enrich jsonb;
begin
  ...
  if v_driver then
    ...
    select coalesce(jsonb_agg(row_to_json(t) order by (t.sync_error is not null) desc, t.created_at), '[]'::jsonb)
      ...
      select s.id as source_id, coalesce(s.display_name, left(s.id::text, 8)) as display_name,
             ...
             (exists (select 1 from cloud_media_items m where m.source_id = s.id and m.item_type in ('movie','series')) and not exists (select 1 from cloud_title_variants v where v.source_id = s.id)) as incomplete,
    ...
    select coalesce(jsonb_agg(row_to_json(t)), '[]'::jsonb) into v_enrich from (
      select coalesce(s.display_name, left(s.id::text, 8)) as panel, ct.item_type, count(*) as total,
      ...
      group by s.display_name, s.id, ct.item_type order by s.display_name, ct.item_type
    ) t;
  end if;
