...
begin
  ...
  if not exists (
    ...
    where constraint_state.conrelid = 'public.catalog_series_episode_memberships'::regclass
      and constraint_state.conname = 'catalog_series_memberships_gen_parent_natural_fk'
  ) then
    alter table public.catalog_series_episode_memberships
      add constraint catalog_series_memberships_gen_parent_natural_fk
  ...
  perform public.norva_assert_catalog_generation_contract_constraint(
    'catalog_series_memberships_gen_parent_natural_fk', false
  );
end
$install$;
