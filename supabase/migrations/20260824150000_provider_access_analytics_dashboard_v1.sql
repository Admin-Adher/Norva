...
-- Phase 15 exposes aggregate operational evidence only. It never returns a
-- user/source/transition identifier, provider metadata, credential material,
-- recipient address or device token.
...
begin
  ...
  select jsonb_build_object(
    '7d_sent', count(distinct notification.access_cycle_id) filter (
      where notification.event_kind = 'expiry_7d' and notification.state = 'completed'
    ),
    '1d_sent', count(distinct notification.access_cycle_id) filter (
      where notification.event_kind = 'expiry_1d' and notification.state = 'completed'
    ),
    'today_sent', count(distinct notification.access_cycle_id) filter (
      where notification.event_kind = 'expiry_today' and notification.state = 'completed'
    ),
    ...
    'push_delivered', count(*) filter (
      where notification.channel = 'push' and notification.state = 'completed'
    ),
    'email_delivered', count(*) filter (
      where notification.channel = 'email' and notification.state = 'completed'
    )
...
create or replace function public.norva_assert_provider_access_rollout_safe()
returns jsonb
language plpgsql
security definer
set search_path = ''
...
declare
  v_snapshot jsonb;
begin
  v_snapshot := public.norva_provider_access_analytics_dashboard(30);
  ...
  return jsonb_build_object(
    'safe', true,
    'checkedAt', v_snapshot -> 'generatedAt',
    'stagingVisibilityViolation', 0
  );
end
$function$;

revoke all on function public.norva_provider_access_analytics_dashboard(integer),
  public.norva_assert_provider_access_rollout_safe()
...
comment on function public.norva_provider_access_analytics_dashboard(integer) is
  'Phase 15 aggregate-only Provider Access dashboard; no user-level or credential data.';
comment on function public.norva_assert_provider_access_rollout_safe() is
  'Phase 16 hard gate: raises P0001 whenever a staging source is catalog-visible.';
