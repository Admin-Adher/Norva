-- EXYU is a regional catalogue fallback, never a claimed audio language.
-- Preserve the audited NL | HINDI declaration and all exact-file precedence.
begin;
...
begin
  ...
  for idx in 1..3 loop
    ...
    annotated := idx <> 2;
    if annotated then part := regexp_replace(part,'^([[:alpha:]ऀ-ൿ]{4,30})[- ]language\s+version$','\1','i'); end if;
    ...
    foreach token in array tokens loop
      ...
      if public.catalog_provider_language_alias(lower_token) is null and lower_token not in ('hu','exyu')
        and not (idx=1 and provider_prefix and lower_token ~ '^(af|vp|eg|ye|s|shr|as|sbus|sham|ma|alg|kh|doc|d|tn|xmas|pod|sh|anm|hara|li|ly|dz|ptv|do|ch|chr|irq|isl|bdy|kid|kids|hdr|dv|jo|jor|cam|dsc|pse|sus|geo)$')
...
-- Widen only the private catalogue-hint constraint. EXYU is not a canonical
-- audio language, an observation, a track map or a playback preference.
alter table public.cloud_catalog_provider_language_hints
  drop constraint cloud_catalog_provider_language_hints_language_check,
  add constraint cloud_catalog_provider_language_hints_language_check
    check (language ~ '^([a-z]{2,3}|nordic|exyu)$');
commit;
