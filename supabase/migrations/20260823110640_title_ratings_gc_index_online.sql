...
begin
  ...
  if to_regclass('public.cloud_title_ratings_title_owner_gc_idx') is not null
     and not public.norva_title_gc_index_is_exact(
       'cloud_title_ratings_title_owner_gc_idx',
       'public.cloud_title_ratings', array['title_id','user_id'], true
     ) then
    if public.norva_title_gc_index_is_exact(
      'cloud_title_ratings_title_owner_gc_idx',
      'public.cloud_title_ratings', array['title_id','user_id'], false
    ) then raise exception 'exact title-rating GC index is invalid; operator must REINDEX INDEX CONCURRENTLY then retry' using errcode='55000';
    else raise exception 'title-rating GC index homonym has wrong shape' using errcode='55000'; end if;
  end if;
end
$preflight$;
create index concurrently if not exists cloud_title_ratings_title_owner_gc_idx
  on public.cloud_title_ratings(title_id,user_id);
do $postcondition$ begin
  if not public.norva_title_gc_index_is_exact(
    'cloud_title_ratings_title_owner_gc_idx',
    'public.cloud_title_ratings', array['title_id','user_id'], true
  ) then raise exception 'title-rating GC index postcondition failed' using errcode='55000'; end if;
end $postcondition$;
reset lock_timeout;
