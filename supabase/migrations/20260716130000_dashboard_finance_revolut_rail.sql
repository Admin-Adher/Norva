-- =============================================================================
-- Dashboard finance → rail-agnostic (Revolut-aware) — G2/G3 du healthcheck 2026-07-16.
-- =============================================================================
-- Confirmé en prod (07-selfhost-data-healthcheck §C) : billing_mrr_cents (cockpit)
-- et mrr_cents (sparklines) faisaient un INNER JOIN cloud_stancer_customers — le
-- rail web Revolut et le rail mobile RC (dont le prix vit sur la PROJECTION via
-- mrr_cents/bill_period, stampés par norva-revolut-billing:185 et le webhook RC)
-- étaient invisibles : MRR affiché 0 dès la première conversion Revolut. Même
-- pattern coalesce() que admin_finance (20260705110000, déjà rail-agnostic).
-- Et billing_cron_fails_24h comptait le cron RETIRÉ norva-stancer-billing
-- (20260711200000) au lieu de norva-revolut-billing → échecs du moteur de revenu
-- invisibles au cockpit. Les deux fonctions sont ré-émises telles quelles avec
-- CES SEULES modifications (générées par script depuis les migrations sources).
-- Le ledger cash (collected/recent payments) est corrigé côté edge : norva-
-- revolut-billing journalise désormais chaque charge capturée dans
-- cloud_stancer_payments (provider='revolut'), comme le webhook RC (rc_*).
...
begin
  insert into admin_metrics_daily(day, metric, value) values
    ...
    (t,'mrr_cents',          (select coalesce(sum(case when coalesce(c.period, p.bill_period)='annual' then round(coalesce(c.amount_cents, p.mrr_cents)/12.0) else coalesce(c.amount_cents, p.mrr_cents) end),0)
                                from cloud_entitlement_projection p left join cloud_stancer_customers c on c.user_id=p.user_id
                                where p.status in ('active','past_due','grace','cancelled_at_period_end'))),
