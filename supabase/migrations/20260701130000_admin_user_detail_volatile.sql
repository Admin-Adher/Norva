-- Hotfix: 20260701120000 declared admin_user_detail STABLE, but it runs `set local
-- statement_timeout` — Postgres forbids SET inside a non-volatile function (SQLSTATE 0A000
-- "SET is not allowed in a non-volatile function"), so every admin click on a user row 400'd.
-- Recreate it VOLATILE (the default — same as refresh_admin_dashboard, which is why that one works).
-- Only the volatility marker changes; the body is identical to 20260701120000.
