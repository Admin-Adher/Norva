...
create policy "cloud_title_variants_owner_select" on public.cloud_title_variants for select to authenticated
using (user_id=(select auth.uid()) and public.norva_source_catalog_visible(source_id,user_id));
do $assert$ begin if not public.norva_provider_access_foundation_policy_is_exact('public.cloud_title_variants','cloud_title_variants_owner_select','r','e1aa3c97babf17d80d236269212b84c920239a68a4388cb210a0e02d5f19270d','e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855') then raise exception 'cloud_title_variants lifecycle policy drift' using errcode='55000'; end if; end $assert$;
commit;
