-- Add real login events to the Clients activity chart: distinct users/day that
-- obtained a refresh token (created on each sign-in). Kept alongside the
-- watch-activity series so the CRM shows both "connexions" and "visionnage".
-- NB: refresh_tokens are pruned by GoTrue, so history is short now but grows
-- forward; watch-activity remains the fuller signal.
...
begin
  ...
  with days as (
    select gs::date as day from generate_series(current_date - (d - 1), current_date, interval '1 day') gs
  ),
  active as (
    select updated_at::date as day, count(distinct user_id) as n
    from cloud_watch_history where updated_at >= current_date - (d - 1) group by 1
  ),
  logins as (
    select created_at::date as day, count(distinct user_id) as n
    from auth.refresh_tokens where created_at >= current_date - (d - 1) group by 1
  ),
  crn as (
    select start_time::date as day, count(*) as runs, count(*) filter (where status = 'failed') as failed
    from cron.job_run_details where start_time >= current_date - (d - 1) group by 1
  ),
  tot as (
  ...
  select jsonb_build_object(
    ...
    'logins_daily', (select coalesce(jsonb_agg(
        jsonb_build_object('day', to_char(dd.day, 'YYYY-MM-DD'), 'logins', coalesce(l.n, 0)) order by dd.day), '[]'::jsonb)
      from days dd left join logins l on l.day = dd.day),
