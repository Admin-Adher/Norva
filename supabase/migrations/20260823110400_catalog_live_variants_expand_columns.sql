...
do $assert$
begin
  if not (select bool_and(public.norva_catalog_expand_column_is_exact('public.cloud_live_variants', expected.column_name, expected.type_name)) from (values
  ...
  ) expected(column_name,type_name)) then raise exception 'cloud_live_variants expand column drift' using errcode = '55000'; end if;
end
$assert$;
commit;
