-- =============================================================================
-- TVA notifications (1/2) : signaux OSS dans le cache pour le cron d'alertes.
-- =============================================================================
-- Le sweep d'alertes (norva-admin /ops-alert, 15 min) lit UNIQUEMENT
-- admin_dashboard_cache.overview (design « cache-only, pas cher »). Pour lui
-- donner de quoi pousser 2 nudges TVA sans casser ce principe, on ajoute 3 clés
-- au blob overview (montants en cents USD — le sweep convertit ~0,92 pour la
-- jauge de seuil, comme le front) :
--   • vat_ytd_eu_cross_cents / vat_prevy_eu_cross_cents : ventes web B2C UE
--     transfrontalières (hors FR) de l'année en cours / précédente → alerte
--     « seuil 10 000 € à 80 % » (le seuil s'apprécie sur N ET N-1) ;
--   • vat_fx_pending : libellé 'Tn AAAA' du dernier trimestre CLOS ayant des
--     ventes UE mais pas de taux BCE figé (oss_fx_rates) → alerte « trimestre
--     clos, figez le taux » ; NULL sinon (→ l'alerte se résout d'elle-même dès
--     que le taux est figé).
-- Ré-émission VERBATIM de refresh_admin_dashboard (base : 20260717120000) avec
-- CES 3 SEULES additions (diff vérifié). Exécuter en supabase_admin.
-- oss_fx_rates doit exister (20260717160000, antérieure).
