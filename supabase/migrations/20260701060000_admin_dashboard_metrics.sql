-- Admin dashboard — richer enrichment/scan metrics.
--
-- Adds, per panel × item_type: resolved %, the never-scanned pool, 24h throughput and an ETA
-- (days to sweep the remaining pool at the current rate). And per cron job: its window
-- (jour 6-23 / nuit 0-5) and dimension (audio / séries / sous-titres / whisper / tmdb / …),
-- so the day/night scan fleet is legible at a glance. Only the coverage + cron sub-selects of
-- refresh_admin_dashboard change; overview + sources are unchanged.
...
begin
  ...
  select jsonb_build_object(
    ...
    'cron_fails_24h',(select count(*) from cron.job_run_details where status <> 'succeeded' and start_time > now() - interval '24 hours')
  ) into ov;
  ...
  select coalesce(jsonb_agg(row_to_json(t)), '[]'::jsonb) into crn from (
    select j.jobid, j.jobname, j.schedule, j.active,
           ...
           coalesce((select count(*) from cron.job_run_details d where d.jobid = j.jobid and d.status <> 'succeeded' and d.start_time > now() - interval '24 hours'), 0) as fails_24h
    from cron.job j
