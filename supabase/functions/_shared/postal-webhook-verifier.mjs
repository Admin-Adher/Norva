...
export async function verifyPostalWebhook(rawBytes, headers, trustedKeys = {}, nowMs = Date.now()) {
  try {
    if (!(rawBytes instanceof Uint8Array) || rawBytes.length > 32768) return { valid: false };
    ...
    return { valid: true, eventId: event.uuid, event: event.event, providerMessageId: message.id,
      ...
      deliveryKey: /^norva-postal-canary-[0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12}$/.test(message.tag ?? '') ? message.tag : null,
