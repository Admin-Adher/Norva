"""Install the observed-session grant, dormant, through the idle-only supervisor.

No subtitle cancellation, pilot activation, provider I/O, model change or reset.
"""
...
ROOT=pathlib.Path('/home/adrien/.norva/passive-observed-grant-20260913')
PARENT=ROOT.parent/'enrichment-admission-20260913-retry1'
NATIVE=ROOT.parent/'passive-live-canary-native-20260913/native-proof.json'
IMAGE='norva-media-gateway:passive-observed-grant-20260913'
FILES=('index.js','enrichment-pilot-admission.js')
...
live=load('observed_grant_parent',PARENT/'deploy-enrichment-admission-20260913.py')
op=load('observed_grant_supervisor',ROOT.parent/'scoped-passive-dormant-20260912/deploy-scoped-passive-dormant-20260912.py')
...
def source_snapshot():
    return live.live.source_snapshot()
...
def stage():
    ...
    require(proof.get('exitCode')==0 and proof.get('counts')=={'tests':40,'pass':40,'fail':0,'skipped':0}
    ...
    base_tag='norva-passive-observed-grant-base:20260913'
    ...
    paths += [PARENT/name for name in ('plan.private.json','receipt.private.json','closed.private.json',
        'subtitle-authorization.private.json','subtitle-cancelled.private.json',
        'deploy-enrichment-admission-20260913.py','deploy-admission-subtitle-maintenance-20260913.py')]
...
if __name__=='__main__':
    ...
    try:
        ...
        code=str(error);print(json.dumps({'ok':False,'code':code if re.fullmatch('[a-zA-Z0-9_:.-]{1,120}',code)
            else 'observed_grant_release_failed'}));sys.exit(1)
