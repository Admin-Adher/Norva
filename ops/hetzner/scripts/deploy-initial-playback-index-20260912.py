...
ROOT = pathlib.Path('/home/adrien/.norva/initial-playback-index-20260912')
CURRENT = ROOT.parent/'resume-cache-retention-20260912'
NATIVE = ROOT.parent/'initial-playback-index-native-20260912-r5/native-proof.json'
IMAGE = 'norva-media-gateway:initial-playback-index-20260912'
...
live = load('initial_index_current', CURRENT/'deploy-resume-cache-retention-20260912.py')
op = load('initial_index_supervisor', ROOT.parent/'scoped-passive-dormant-20260912/deploy-scoped-passive-dormant-20260912.py')
op.ROOT, op.NATIVE, op.FILES, op.IMAGE = ROOT, NATIVE, FILES, IMAGE
op.PREFIX = op.SERVICE+'-initial-playback-index-20260912'
...
def verify_gateway(plan, candidate):
    ...
    if candidate:
        ...
        require(index.get('protocol') == 1 and index.get('maxBytes') == 32*1024*1024
...
def stage():
    ...
    require(proof.get('exitCode') == 0 and proof.get('counts') == {'tests':109, 'pass':108, 'fail':0, 'skipped':1}
    ...
    paths += [CURRENT/name for name in ('plan.private.json', 'receipt.private.json', 'closed.private.json', 'deploy-resume-cache-retention-20260912.py')]
