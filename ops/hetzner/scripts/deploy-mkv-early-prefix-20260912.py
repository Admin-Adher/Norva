"""One Gateway module: validated early local prefix, unchanged provider controls."""
import importlib.util,json,os,pathlib,re,sys,tarfile,time
ROOT=pathlib.Path('/home/adrien/.norva/mkv-early-prefix-20260912')
NATIVE=ROOT.parent/'mkv-early-prefix-native-20260912/native-proof.json'
FILES=('index.js',)
IMAGE='norva-media-gateway:mkv-early-prefix-20260912'

def load(name,path):
    s=importlib.util.spec_from_file_location(name,path);m=importlib.util.module_from_spec(s);sys.modules[name]=m;s.loader.exec_module(m);return m
current=load('mkv_early_current',ROOT.parent/'mkv-audio-alignment-20260912/deploy-mkv-audio-alignment-20260912.py')
op=load('mkv_early_supervisor',ROOT.parent/'scoped-passive-dormant-20260912/deploy-scoped-passive-dormant-20260912.py')
op.ROOT,op.NATIVE,op.FILES,op.IMAGE=ROOT,NATIVE,FILES,IMAGE
op.PREFIX=op.SERVICE+'-mkv-early-prefix-20260912';op.__file__=__file__
gw,require=op.gw,op.require
source_snapshot=current.source_snapshot

def invariant(plan):
    # Current invariant binds only protected records, controls, Edge and other
    # containers; its old Gateway identity must not be reused for this release.
    current.invariant(plan)
...
def stage():
    ...
    current.op.verify();original=gw.inspect(op.SERVICE);gw.assert_image_backed_runtime(original)
    proof=json.loads(NATIVE.read_text())
    require(proof.get('exitCode')==0 and proof.get('counts')=={'tests':104,'pass':104,'fail':0,'skipped':0}
    ...
    tag='norva-mkv-early-prefix-base:20260912';gw.run(['docker','tag',original['Image'],tag])
    ...
    current.op.verify();parent=current.op.saved('plan.private.json')
    ...
    plan={'commit':commit,'original':original,'before':before,'after':after,
        ...
        'otherContainers':{name:gw.inspect(name)['Id'] for name in parent['otherContainers']},
...
if __name__=='__main__':
    ...
    try:
        ...
        code=str(error);print(json.dumps({'ok':False,'code':code if re.fullmatch('[a-zA-Z0-9_:.-]{1,120}',code) else 'mkv_early_prefix_release_failed'}),flush=True);sys.exit(1)
