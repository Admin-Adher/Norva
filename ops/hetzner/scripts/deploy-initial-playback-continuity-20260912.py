...
ROOT = pathlib.Path('/home/adrien/.norva/initial-playback-continuity-20260912')
CURRENT = ROOT.parent/'initial-playback-id3-20260912'
NATIVE = ROOT.parent/'initial-playback-index-native-20260912-r7/native-proof.json'
IMAGE = 'norva-media-gateway:initial-playback-continuity-20260912'
...
live = load('initial_continuity_current', CURRENT/'deploy-initial-playback-id3-20260912.py')
op = load('initial_continuity_supervisor', ROOT.parent/'scoped-passive-dormant-20260912/deploy-scoped-passive-dormant-20260912.py')
op.ROOT, op.NATIVE, op.FILES, op.IMAGE = ROOT, NATIVE, FILES, IMAGE
op.PREFIX = op.SERVICE+'-initial-playback-continuity-20260912'
...
def verify_gateway(plan, candidate):
    ...
    if candidate:
        ...
        require(index.get('protocol') == 1 and index.get('parserRevision') == 3 and index.get('maxBytes') == 32*1024*1024
...
def stage():
    ...
    require(len(rows) == 4 and {row.get('encoder') for row in rows} == {'software', 'vaapi'}, 'native_encoders_missing')
    for encoder in ('software', 'vaapi'):
        pair = [row for row in rows if row['encoder'] == encoder]
    ...
    paths += [CURRENT/name for name in ('plan.private.json', 'receipt.private.json', 'closed.private.json', 'deploy-initial-playback-id3-20260912.py')]
