...
--
-- NE PAS coller ce fichier dans bash : exécute-le via le wrapper après merge/checkout :
--   ./ops/hetzner/scripts/15-provider-presence-gate-audit-v3.sh <uuid utilisateur pilote>
-- ou, depuis la racine ~/norva, via l'alias :
--   ./scripts/15-provider-presence-gate-audit-v3.sh <uuid utilisateur pilote>
--
-- Si la box répond "No such file or directory" après git pull alors que la PR est sur main,
-- force la mise à jour de main : `git fetch origin main && git pull --ff-only origin main`,
-- puis vérifie le chemin canonique `ops/hetzner/scripts/15-provider-presence-gate-audit-v3.sh`.
--
