"""Nonblocking ready response and near-origin TS seek: one Gateway file and one Edge file, with retained rollback.
...
ROOT = pathlib.Path('/home/adrien/.norva/ts-quick-start-20260912')
CURRENT = ROOT.parent/'ts-aligned-resume-20260912'
NATIVE = ROOT.parent/'finite-ts-policy-native-20260912-r7/native-proof.json'
APP_COMMIT = '42a2c17e07390c8ccd1a61d09aed020c0462650b'
FILES = ('index.js',)
EDGE_FILE = 'norva-playback/index.ts'
BASE_EDGE_SHA = 'ca5b2bc5d731152b7050cd09acc54b2313af0ce15147989e8e606a86786925a3'
IMAGE = 'norva-media-gateway:ts-quick-start-20260912'
...
live = load('ts_quick_start_current', CURRENT/'deploy-ts-aligned-resume-20260912.py')
op = load('ts_quick_start_supervisor', ROOT.parent/'scoped-passive-dormant-20260912/deploy-scoped-passive-dormant-20260912.py')
op.ROOT, op.NATIVE, op.APP_COMMIT, op.FILES, op.IMAGE = ROOT, NATIVE, APP_COMMIT, FILES, IMAGE
op.PREFIX = op.SERVICE+'-ts-quick-start-20260912'
...
def stage():
    ...
    paths += [CURRENT/n for n in ('plan.private.json','receipt.private.json','closed.private.json','deploy-ts-aligned-resume-20260912.py')]
...
def activate_edge(name, plan):
    ...
    candidate_name = name+'-ts-quick-start-20260912-candidate'
    ...
    gw.run(['docker','rename',original['Id'],name+'-ts-quick-start-20260912-retained'])
...
if __name__ == '__main__':
    ...
    try:
        ...
        code = str(error)
        print(json.dumps({'ok':False,'code':code if re.fullmatch('[a-zA-Z0-9_:.-]{1,120}',code) else 'ts_quick_start_release_failed'}),flush=True)
        sys.exit(1)
