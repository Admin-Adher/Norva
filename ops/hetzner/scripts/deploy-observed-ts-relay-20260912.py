...
ROOT=pathlib.Path('/home/adrien/.norva/observed-ts-relay-20260912')
FILE='norva-playback/index.ts'
BASE_SHA='7cf28e263434dfd1f91166350f07c705446fc4f995f630d00b321ed776a32717'
...
live=load('observed_ts_relay_live',ROOT.parent/'ts-quick-start-20260912/deploy-ts-quick-start-20260912.py')
previous=load('observed_ts_relay_previous',ROOT.parent/'observed-container-hint-20260912/deploy-observed-container-hint-20260912.py')
op=load('observed_ts_relay_supervisor',ROOT.parent/'scoped-passive-dormant-20260912/deploy-scoped-passive-dormant-20260912.py')
...
def stage():
    ...
    parent_plan=previous.op.saved('plan.private.json')
    paths=[pathlib.Path(n) for n in parent_plan['protectedFiles']]
    paths += [previous.ROOT/n for n in ('plan.private.json','closed.private.json')]
...
def deploy(plan):
    ...
    for name in EDGES:
        ...
        original=plan['edges'][name];candidate_name=name+'-observed-ts-relay-20260912-candidate'
        ...
        gw.run(['docker','rename',original['Id'],name+'-observed-ts-relay-20260912-retained'])
...
if __name__=='__main__':
    ...
    try:
        ...
        code=str(error);print(json.dumps({'ok':False,'code':code if re.fullmatch('[a-zA-Z0-9_:.-]{1,120}',code) else 'observed_ts_relay_release_failed'}),flush=True);sys.exit(1)
