"""TS serialized resume and private error diagnostics: one Gateway file and one Edge file, with retained rollback.
...
ROOT = pathlib.Path('/home/adrien/.norva/startup-subtitle-20260912')
CURRENT = ROOT.parent/'mkv-early-prefix-20260912'
NATIVE = ROOT.parent/'startup-subtitle-native-20260912-r4/native-proof.json'
APP_COMMIT = ''
FILES = ('index.js',)
EDGE_FILE = 'norva-playback/index.ts'
BASE_EDGE_SHA = 'df09ebe9407e371e8e97ab5dc9060ad44544b4bb2e99057292ccd7c56a654573'
IMAGE = 'norva-media-gateway:startup-subtitle-20260912'
...
live = load('startup_subtitle_current', CURRENT/'deploy-mkv-early-prefix-20260912.py')
op = load('startup_subtitle_supervisor', ROOT.parent/'scoped-passive-dormant-20260912/deploy-scoped-passive-dormant-20260912.py')
op.ROOT, op.NATIVE, op.APP_COMMIT, op.FILES, op.IMAGE = ROOT, NATIVE, APP_COMMIT, FILES, IMAGE
op.PREFIX = op.SERVICE+'-startup-subtitle-20260912'
...
def stage():
    ...
    require(proof.get('exitCode') == 0 and proof.get('counts') == {'tests':60,'pass':1,'fail':0,'skipped':59}
    ...
    base_tag = 'norva-ts-quick-start-base:20260912'
    ...
    paths += [CURRENT/n for n in ('plan.private.json','receipt.private.json','closed.private.json','deploy-mkv-early-prefix-20260912.py')]
    ...
    print(json.dumps({'staged':True,'productionUnchanged':True,'gatewayModules':1,'edgeFiles':1,
...
def activate_edge(name, plan):
    ...
    candidate_name = name+'-startup-subtitle-20260912-candidate'
    ...
    gw.run(['docker','rename',original['Id'],name+'-startup-subtitle-20260912-retained'])
...
def verify():
    ...
    for name in EDGES: verify_edge(name,plan,success)
    require(op.base.r.crons() == plan['crons'],'cron_restore_missing')
    print(json.dumps({'verified':True,'productionUpdated':success,'commit':plan['commit'],'newProviderRequests':0,
        'cronsRestored':True,'passiveEnabled':False,'gatewayModules':1,'edgeFiles':1}))
...
if __name__ == '__main__':
    ...
    try:
        ...
        code = str(error)
        print(json.dumps({'ok':False,'code':code if re.fullmatch('[a-zA-Z0-9_:.-]{1,120}',code) else 'startup_subtitle_release_failed'}),flush=True)
        sys.exit(1)
