...
def main():
    ...
    name = 'norva-resume-reuse-native-20260912'
    ...
    with log.open('x') as output:
        process = subprocess.Popen(['docker', 'run', '--name', name, '--rm', '--network', 'none',
            ...
            '-e', 'NODE_PATH=/app/node_modules', '-e', 'NORVA_TS_BROKER_NATIVE=1', '--entrypoint', 'node', image,
            '--test', '--test-reporter=tap', '--test-name-pattern=native finite TS far seek',
            '/test/tests/media-gateway-strict-lid-broker.test.js'], stdout=output, stderr=output)
        at = time.monotonic()
    ...
    print('\n'.join(line for line in text.splitlines() if 'native TS broker metrics' in line))
    if code:
        print(text[-6000:])
    sys.exit(code)
