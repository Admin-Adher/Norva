"""Deploy only email coverage; preserve other live files and unrelated configuration."""
...
assert root.parent==pathlib.Path('/home/adrien/.norva') and root.name.startswith('email-coverage-'), 'Unexpected release directory'
manifest=json.loads((root/'candidate'/'manifest.json').read_text())
expected={r:v['before']for r,v in manifest.items()}
flags={'NORVA_LC_WINBACK':'true','NORVA_LC_ABANDONED':'true','NORVA_OPS_EMAIL':'buildtrack.admin@gmail.com'}
...
if phase=='stage':
 ...
 try:
  ...
  print(json.dumps({'deployed':name,'healthy':True,'hashesVerified':True,'unrelatedConfigurationPreserved':True,'emailFlags':flags}))
 except Exception:
