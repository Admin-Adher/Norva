...
ROOT = pathlib.Path('/home/adrien/.norva/initial-playback-idr-20260912')
CURRENT = ROOT.parent/'initial-playback-continuity-20260912'
NATIVE = ROOT.parent/'initial-playback-index-native-20260912-r8/native-proof.json'
IMAGE = 'norva-media-gateway:initial-playback-idr-20260912'
FILES = ('index.js', 'finite-ts-seek-index.js', 'finite-ts-landmarks.js')
...
live = load('initial_idr_current', CURRENT/'deploy-initial-playback-continuity-20260912.py')
op = load('initial_idr_supervisor', ROOT.parent/'scoped-passive-dormant-20260912/deploy-scoped-passive-dormant-20260912.py')
op.ROOT, op.NATIVE, op.FILES, op.IMAGE = ROOT, NATIVE, FILES, IMAGE
op.PREFIX = op.SERVICE+'-initial-playback-idr-20260912'
...
def source_snapshot(container=None):
    ...
    result = live.source_snapshot(container)
...
def verify_gateway(plan, candidate):
    ...
    if candidate:
        ...
        require(cache.get('maxBytes') == 64*1024*1024 and cache.get('ttlMs') == 30*60_000
            and cache.get('scope') == 'process-private-owner-exact-source'
            and cache.get('maxRetainedWindowBytes') == 1280*1024, 'resume_cache_limits_changed')
        require((health.get('finiteMkvSeekBroker') or {}).get('protocol') == 10, 'resume_broker_protocol_missing')
        index = health.get('finiteTsSeekIndex') or {}
        require(index.get('protocol') == 1 and index.get('parserRevision') == 4 and index.get('maxBytes') == 32*1024*1024
            and index.get('mediaBytesPersisted') == 0 and index.get('ttlMs') == 7*86400000,
            'persistent_index_limits_changed')
...
def stage():
    ...
    require(proof.get('exitCode') == 0 and proof.get('counts') == {'tests':111, 'pass':110, 'fail':0, 'skipped':1}
    ...
    rows = proof.get('indexMetrics', [])
    require(len(rows) == 8 and {row.get('encoder') for row in rows} == {'software', 'vaapi'}, 'native_encoders_missing')
    for encoder in ('software', 'vaapi'):
        for seek_offset in (41, 87):
            pair = [row for row in rows if row['encoder'] == encoder and row.get('seekOffset') == seek_offset]
            require([row['indexed'] for row in pair] == [False, True]
                and all(row['peak'] == 1 and row['frames'] >= 10 and row['audioFrames'] >= 10 for row in pair)
                and pair[1]['requests'] < pair[0]['requests'], 'native_navigation_proof_missing')
    ...
    base_tag = 'norva-initial-playback-index-base:20260912'
    ...
    paths += [CURRENT/name for name in ('plan.private.json', 'receipt.private.json', 'closed.private.json', 'deploy-initial-playback-continuity-20260912.py')]
