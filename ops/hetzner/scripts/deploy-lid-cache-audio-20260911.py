...
the independently executed import and immutable-audio HTTP proofs. Never print Docker Env or SQL rows.
"""
import copy
import hashlib
...
ROOT = pathlib.Path('/home/adrien/.norva/lid-cache-audio-release-20260911/candidate-1')
...
gw.MODULES = ('index.js', 'strict-lid-speech-sampler.js')
SERVICES = ('norva-media-gateway', 'norva-edge-functions', 'norva-edge-functions-2')
EDGE_FILE = 'norva-playback/index.ts'
MIGRATIONS = ('supabase/migrations/20260911024716_serialize_exact_file_language_hydration.sql',
              'supabase/migrations/20260911034623_read_owned_movie_language_certificate.sql')
BASE_GATEWAY_SHA = '4c22236d03fd528cc084bd2f016b65930c5305e985e54ddc614bacad010f9e06'
...
def controls():
    ...
    return json.loads(sql("select json_build_object('benchmark',"
        ...
        "'ownedReadRpc',to_regprocedure('public.read_owned_movie_language_certificate(uuid,uuid,text)') is not null,"
        "'serializedHydrators',(select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace "
        "where n.nspname='public' and p.proname in ('hydrate_catalog_episode_file_tracks','hydrate_cloud_title_file_languages') "
        "and p.prosrc like '%Exact-file hydration serialization v1%'),"
...
def assert_fixture_binding(candidate):
    proof = document('import-proof.json')
    require(proof.get('ok') is True and proof.get('removedOwnContainers') is True
            and proof.get('importReadOnlyProfileCase') is True, 'import_proof_incomplete')
    result = proof.get('importHydration', {})
    require(all(result.get(k) is True for k in ('accepted', 'cacheReuseAfterImportPassed',
            'catalogLanguageReusePassed', 'languageFacetReusePassed', 'differentProviderExcluded',
            'noProviderOrGatewayRequests')), 'import_invariants_failed')
    require(result.get('readOnlyCertificateGuards', {}).get('passed') is True
            and len(result['readOnlyCertificateGuards']['cases']) == 12, 'certificate_guards_incomplete')
    require(proof.get('edgeIndexSha256') == candidate['edge'], 'edge_not_http_tested')
    require([proof.get('hydrationPatchSha256'), proof.get('importProfileReadMigrationSha256')]
            == [candidate['migrations'][name] for name in MIGRATIONS], 'migrations_not_http_tested')
    for filename, count in [('audio-proof.json', 3), ('viewer-proof.json', 2)]:
        audio = document(filename)
        require(audio.get('passed') is True and audio.get('externalProviderRequests') == 0
                and audio.get('originalClipUnchanged') is True, 'audio_proof_incomplete')
        require(len(audio.get('cases', [])) == count and all(
            c.get('passed') is True and c.get('gatewayIdle') is True
            and c.get('providerActive') == 0 and c.get('remainingTemporaryFiles') == []
            for c in audio['cases']), 'audio_lifecycle_failed')
        require(all(audio.get('hashes', {}).get('/app/src/' + name) == sha for name, sha in
                candidate['gateway'].items()), 'gateway_not_http_tested')
    tests = document('node-proof.json')
    require(tests.get('failed') == 0 and tests.get('passed', 0) >= 4243
            and tests.get('candidate') == candidate, 'combined_node_tests_not_bound')
...
def stage():
    ...
    require(before['strict-lid-speech-sampler.js'] ==
            '359c2d415660e4a2aff26930a58012e63108116c5de7b85d42356ce852f38058',
            'speech_sampler_baseline_drift')
    ...
    candidate = {'gateway': {name: digest(read('candidate/services/media-gateway/src/' + name)) for name in gw.MODULES},
                 ...
                 'migrations': {name: digest(read('candidate/' + name)) for name in MIGRATIONS}}
    assert_fixture_binding(candidate)
    state = controls()
    require(state['benchmark'] is False and state['observedColumns'] == 3 and not state['ownedReadRpc'] and state['serializedHydrators'] == 0 and state['quarantinePreserved'], 'database_baseline_drift')
    ...
    image = 'norva-media-gateway:lid-cache-audio-20260911-candidate-1'
    ...
    base_alias = 'norva-lid-cache-audio-base:20260911-candidate-1'
    ...
    private_write(ROOT / 'database-before.private.json', {'hydrationDefinitions': definitions,
                                                         'hydrationAttributes': hydration_attributes()})
...
def hydration_attributes():
    return json.loads(sql("select json_agg(json_build_object('signature',p.oid::regprocedure::text,"
        "'owner',pg_get_userbyid(p.proowner),'acl',p.proacl,'definer',p.prosecdef,'settings',p.proconfig) "
        "order by p.oid::regprocedure::text) from pg_proc p join pg_namespace n on n.oid=p.pronamespace "
        "where n.nspname='public' and p.proname in ('hydrate_catalog_episode_file_tracks','hydrate_cloud_title_file_languages');"))


def database_checks():
    value = json.loads(sql("select json_build_object('ownedRpcProtected',"
        "pg_get_userbyid(proowner)='postgres' and not prosecdef and provolatile='s' "
        "and proconfig=array['search_path=\"\"']::text[] "
        "and not has_function_privilege('anon',oid,'execute') "
        "and not has_function_privilege('authenticated',oid,'execute') "
        "and has_function_privilege('service_role',oid,'execute')) from pg_proc "
        "where oid='public.read_owned_movie_language_certificate(uuid,uuid,text)'::regprocedure;"))
    before = document('database-before.private.json')['hydrationAttributes']
    value['existingHydrationAttributesPreserved'] = len(before) == 4 and hydration_attributes() == before
    value['fourHydratorsSerialized'] = controls()['serializedHydrators'] == 4
    return value


def atomic_migrations(contents):
    # The reviewed inputs are self-contained transactions. Combine their bodies
    # so either both changes commit or neither does; never leave half a release.
    bodies = []
    for raw in contents:
        text = raw.decode()
        require(len(re.findall(r'^begin;\s*$', text, re.I | re.M)) == 1
                and len(re.findall(r'^commit;\s*$', text, re.I | re.M)) == 1,
                'unexpected_transaction_boundary')
        text = re.sub(r'^(begin|commit);[ \t]*$', '', text, flags=re.I | re.M)
        bodies.append(text)
    return 'BEGIN;\n' + '\n'.join(bodies) + '\nCOMMIT;\n'
...
def database():
    ...
    require(hydration_attributes() == document('database-before.private.json')['hydrationAttributes'],
            'database_attributes_changed_after_stage')
    gw.assert_idle(gw.health())
    contents = [read('candidate/' + name) for name in MIGRATIONS]
    require(all(digest(raw) == plan['candidate']['migrations'][name]
                for name, raw in zip(MIGRATIONS, contents)), 'migration_input_changed')
    sql(atomic_migrations(contents), write=True)
    checks = database_checks()
    require(all(checks.values()), 'database_permissions_failed')
    state = controls()
    require(state['observedColumns'] == 3 and state['ownedReadRpc']
            and state['serializedHydrators'] == 4 and state['benchmark'] is False
            and state['quarantinePreserved'], 'database_postcheck_failed')
    result = {'applied': True, 'migrationSha256': plan['candidate']['migrations'],
              'atomic': True, 'checks': checks, 'controls': state}
...
def activate(name):
    ...
    candidate_name = name + '-lid-cache-audio-candidate-20260911-1'
    rollback_name = name + '-lid-cache-audio-rollback-20260911-1'
...
def verify():
    ...
    result = {'productionVerified': True, 'commit': plan['commit'], 'services': checks,
              ...
              'newIndependentVodBatchNotStarted': True, 'checkedAt': time.time()}
