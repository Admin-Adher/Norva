"""One Gateway file: paced TS continuation and bounded abandoned-window drain. Retained rollback, no Edge/SQL changes.
...
ROOT = pathlib.Path('/home/adrien/.norva/ts-seek-drain-20260912')
CURRENT = ROOT.parent/'ts-seek-window-20260912'
NATIVE = ROOT.parent/'ts-seek-drain-native-20260912/native-proof.json'
IMAGE = 'norva-media-gateway:ts-seek-drain-20260912'
FILES = ('index.js',)
...
live = load('ts_window_current_release', CURRENT/'deploy-ts-seek-window-20260912.py')
...
op.PREFIX = op.SERVICE+'-ts-seek-drain-20260912'
...
def verify_gateway(plan, candidate):
    ...
    require(live.live.source_snapshot() == plan['after' if candidate else 'before'], 'gateway_source_changed')
...
def stage():
    ...
    require(proof.get('exitCode') == 0 and proof.get('counts') == {'tests':71,'pass':1,'fail':0,'skipped':70}
    ...
    before = live.live.source_snapshot(); after = {**before,**{name:gw.sha((context/name).read_bytes()) for name in FILES}}
    (context/'Dockerfile').write_text('ARG BASE_IMAGE\nFROM ${BASE_IMAGE}\n'+''.join('COPY --chmod=0644 '+n+' /app/src/'+n+'\n' for n in FILES))
    base_tag = 'norva-ts-seek-drain-base:20260912'
    ...
    paths += [CURRENT/n for n in ('plan.private.json','receipt.private.json','closed.private.json','deploy-ts-seek-window-20260912.py')]
