...
# accident. Audit 2026-08-27 confirmed low B-tree density on the three
# high-churn Cloud tables; a concurrent pass reclaimed about 1.85 GB with no
# invalid index. catalog_titles is TOAST-dominated and deliberately excluded.
#
...
# when this is overdue (12,000 threshold vs ~10,109 fresh after Phase 3).
#
...
# catalog_titles is dominated by TOAST payload (not index bloat); rebuilding its
# tiny index set cannot reclaim that space. Keep the monthly operation scoped to
# the three high-churn Cloud tables whose B-tree density actually degrades.
TABLES="${REINDEX_TABLES:-public.cloud_titles public.cloud_media_items public.cloud_title_variants}"
