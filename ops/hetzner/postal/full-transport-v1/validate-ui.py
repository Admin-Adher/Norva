"""Three explicitly labeled internal checks of the new email presentation."""
import base64,json,pathlib,subprocess
root=pathlib.Path(__file__).resolve().parent
items=[x for x in json.loads((root/'internal-validation.json').read_text()) if x['flow'] in ['welcome','verification']]
assert len(items)==2
for x in items:x['subject']=x['subject'].replace('[TEST Norva]','[TEST Norva - nouveau design]')
...
begin
 ...
 for x in select value from jsonb_array_elements(convert_from(decode('PAYLOAD','base64'),'UTF8')::jsonb) loop
  ...
  values(v_id,'norva-branded-'||v_id,'email-ui-20260908:'||(x->>'flow'),u,'postal_transport_test','pending',
 ...
 perform norva_enqueue_branded_email(v_to,'[TEST Norva - nouveau design] Your Norva password was changed','Password changed — TEST',
  ...
  'postal_transport_test','email-ui-20260908:security_password_changed',u);
end $test$;
select state,count(*) from cloud_branded_email_outbox where dedupe_key like 'email-ui-20260908:%' group by state;
commit;
