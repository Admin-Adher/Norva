...
// Redirect TV user agents from /login.html to /pair.html (QR-based pairing)
app.get('/login.html', (req, res, next) => {
    if ((req.headers['user-agent'] || '').includes('NorvaTV-AndroidTV') || (req.headers['user-agent'] || '').includes('NodeCastTV-AndroidTV')) {
        return res.redirect('/pair.html');
    }
    next();
});
...
function resolveUnpackedBinaryPath(binaryPath) {
    if (!binaryPath || !binaryPath.includes('app.asar')) {
        return binaryPath;
    }
...
function findFFmpeg() {
    ...
    try {
        const ffmpegPath = resolveUnpackedBinaryPath(require('ffmpeg-static'));
...
function findFFprobe() {
    ...
    try {
        const ffprobePath = resolveUnpackedBinaryPath(require('@ffprobe-installer/ffprobe').path);
        if (ffprobePath) {
...
app.listen(PORT, async () => {
    ...
    // Announce hub on local network via mDNS (optional)
    try {
        require('./services/mdns').start(PORT);
    } catch (e) {
        console.warn('mDNS start failed:', e.message);
    }

    // Start background maintenance after the server settles. Do not refresh the
    // whole provider catalog on every desktop launch: it competes with playback
    // and can exhaust one-connection IPTV accounts.
    setTimeout(async () => {
