...
function getDefaultSettings() {
  return {
    ...
    preferredLanguage: '',        // '' (any) | vf | vostfr | multi | vo ... preferred version tag
...
async function renameWithRetry(source, destination, maxAttempts = 5) {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      await fs.rename(source, destination);
      return;
    } catch (err) {
      const canRetry = ['EPERM', 'EBUSY', 'EACCES'].includes(err.code);
...
async function saveDb(data) {
  ...
  writeQueue = writeQueue.then(async () => {
    const tmpPath = `${dbPath}.${process.pid}.${Date.now()}.${Math.random().toString(16).slice(2)}.tmp`;
    try {
