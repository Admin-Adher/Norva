...
router.get('/status', async (req, res) => {
    try {
        res.json(syncService.getSyncStatuses());
    } catch (err) {
        console.error('Error getting sync status:', err);
        res.json(syncService.getCachedSyncStatuses());
    }
});
...
// Manual Hard Sync: clear local content for this source, then sync from scratch.
router.post('/:id/hard-sync', async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        const source = await sources.getById(id);
        if (!source) return res.status(404).json({ error: 'Source not found' });

        const result = await syncService.hardRefreshSource(id);
        ...
        console.error('Error starting hard sync:', err);
        const status = /already syncing/i.test(err.message) ? 409 : 500;
        res.status(status).json({ error: 'Failed to start hard refresh', message: err.message });
    }
});
