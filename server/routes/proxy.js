...
function getStreamsFromDb(sourceId, type, categoryId = null, includeHidden = false) {
    const db = getDb();
    let query = `
        SELECT item_id, name, stream_icon, stream_url, added_at, rating, container_extension, year, category_id,
               dedup_key, tmdb_id, tmdb_data, playback_status, playback_failures,
               playback_last_error, playback_checked_at, playback_mode, playback_mode_reason,
               playback_mode_checked_at, data
        FROM playlist_items
        WHERE source_id = ? AND type = ?
    `;
    if (!includeHidden) {
    ...
    return items.map(item => {
        ...
        // Parsed TMDB enrichment (genres, runtime, overview, poster...)
        let tmdb = null;
        if (item.tmdb_data) {
            try {
                const parsed = JSON.parse(item.tmdb_data);
                if (parsed && parsed.matched) tmdb = parsed;
            } catch (e) { /* ignore malformed tmdb_data */ }
        }
        ...
        return {
            ...
            tmdb,
            playback_status: item.playback_status || 'unknown',
            playback_failures: item.playback_failures || 0,
            playback_last_error: item.playback_last_error || null,
            playback_checked_at: item.playback_checked_at || null,
            playback_mode: item.playback_mode || 'unknown',
            playback_mode_reason: item.playback_mode_reason || null,
            playback_mode_checked_at: item.playback_mode_checked_at || null,
            stream_url: item.stream_url || data.stream_url || data.url || null,
...
router.get('/stream', async (req, res) => {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (!response.ok) {
                console.error(`Upstream error for ${sanitizeErrorMessage(url).substring(0, 80)}...: ${response.status} ${response.statusText}`);
            ...
            if (contentLooksLikeHls) {
                ...
                console.log(`[Proxy] Processing HLS manifest from: ${sanitizeErrorMessage(finalUrl).substring(0, 80)}...`);
