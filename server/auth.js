...
const JWT_SECRET = process.env.JWT_SECRET || 'norva-secret-key-change-in-production';
...
/**
 * Generate a long-lived device token for a paired TV/mobile device
 */
function generateDeviceToken(user, deviceId, deviceType) {
    return jwt.sign(
        {
            id: user.id,
            username: user.username,
