...
test('the direct public decision runs within existing authorization and session lifecycle, with an exclusive marker', () => {
  ...
  assert.match(create, /const mode = serverOwnedEpisodeGateway\s*\? "transcode"\s*: serverDirectPublicHls\s*\? "direct"\s*: serverNativeProviderMp4\s*\? "relay"\s*: serverPromotedProviderMp4\s*\? "transcode"\s*: serverDemotedAutomaticMp4/);
