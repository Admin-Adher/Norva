...
test('only the 14 individually tested files import, even when remote catalogues are unavailable', async () => {
  const { SELECTION_TESTED_VOD } = await import('../supabase/functions/_shared/selection-tested-vod.mjs');
  ...
  assert.equal(rows.filter(row => row.item_type === 'movie').length, 11);
  assert.equal(rows.filter(row => row.item_type === 'episode').length, 3);
  assert.equal(rows.filter(row => row.item_type === 'series').length, 3);
  assert.equal(new Set(rows.map(row => row.external_id)).size, 17);
  assert.deepEqual(sources.filter(source => source.status === 'loaded').map(source => source.included), [5, 5, 4]);
  ...
  assert.equal(rows.find(row => row.title === 'Nobody (2021)').playback_hint.container, 'mkv');
