...
test('capture is encrypted, private, bound to the exact file and durable across a fresh store instance', async t => {
    ...
    assert.equal(disk.includes(Buffer.from('private-owner')), false);
    assert.equal(disk.includes(Buffer.from('RIFF')), false);
    assert.equal(disk.includes(bytes.subarray(100, 200)), false);
