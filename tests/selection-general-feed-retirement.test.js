...
test('retiring the general feed preserves Movies and other TV/VOD sources without fetching the retired playlist', async () => {
  ...
  assert.ok(!DISCOVERY_SOURCES.some(feed => feed.id === 'iptv-org'));
  assert.equal(DISCOVERY_SOURCES.find(feed => feed.id === 'iptv-org-movies').kind, 'live');
  const feeds = [
    { id: 'iptv-org', name: 'IPTV-org', kind: 'live', url: 'general' },
    ...['iptv-org-movies', 'free-tv', 'publicdomain'].map(id => DISCOVERY_SOURCES.find(feed => feed.id === id)),
  ];
  const called = [];
...
test('stale general links are refused only in the owned Selection; cinema and personal playlists still resolve', async () => {
  const { resolveDiscoveryTarget } = await import(modulePath);
  ...
  await assert.rejects(resolveDiscoveryTarget(options), /unavailable/);
  assert.equal(await resolveDiscoveryTarget({ ...options, sourceId: 'personal-source' }), options.targetUrl);
  assert.equal(await resolveDiscoveryTarget({ ...options, userId: 'different-owner' }), options.targetUrl);
  assert.equal(await resolveDiscoveryTarget({ ...options, metadata: { ...options.metadata, group: 'IPTV-org · Movies;Series' } }), options.targetUrl);
  assert.equal(await resolveDiscoveryTarget({ ...options, metadata: { discoveryFeed: 'iptv-org-movies' } }), options.targetUrl);
});
