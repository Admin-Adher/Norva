...
function handoffHarness({ missing = false, writeFails = false, superseded = false, imported = true } = {}) {
    const events = [], filters = [];
    let written;
    const hint = { unrelated: 'preserved', lastSync: { total: 80 },
        syncProgress: { counts: { total: 7697 }, steps: { import: { status: imported ? 'done' : 'running' } } } };
    const db = { from(table) {
...
test('the real activation importer hands off Selection before any whole-catalogue projection', async () => {
    ...
    const context = vm.createContext({
        ...
        discoveryCatalogFields: (_, item) => ({ item_type: item.item_type }),
        ...
        refreshVodTitleProjection: async () => { projected.push('vod'); },
    });
