...
test('catalog rollout proves the exact-file codec protocol on every Edge replica', () => {
  ...
  assert.match(catalog, /version:\s*8/);
  assert.match(catalog, /flatCodecProfileProtocol:\s*1/);
  assert.match(catalog, /catalogLanguageFacetProtocol:\s*2/);
  assert.match(catalog, /boundedMediaHydrationProtocol:\s*1/);
  assert.match(catalog, /exactTrackPersistenceProtocol:\s*2/);
  assert.match(deploy, /EXPECTED_CATALOG_VERSION=8/);
  assert.match(deploy, /EXPECTED_FLAT_CODEC_PROFILE_PROTOCOL=1/);
  assert.match(deploy, /EXPECTED_CATALOG_LANGUAGE_FACET_PROTOCOL=2/);
  assert.match(deploy, /EXPECTED_BOUNDED_MEDIA_HYDRATION_PROTOCOL=1/);
  ...
  assert.match(app, /\/js\/api\.js\?v=[0-9a-f]+/);
  assert.match(app, /\/js\/pages\/WatchPage\.js\?v=[0-9a-f]+/);
});
