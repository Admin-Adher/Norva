...
test('connected-product navigation links both shipped Android apps to their canonical Play Store listings', () => {
  ...
  assert.match(appHtml, /\/js\/app\.js\?v=b39c6d5e2a/);
});
...
test('all changed funnel assets use fresh cache keys', () => {
  ...
  assert.match(appHtml, /cloudApi\.js\?v=65/);
  assert.match(appHtml, /Settings\.js\?v=52/);
  ...
  assert.match(appHtml, /HomePage\.js\?v=60/);
  assert.match(appHtml, /app\.js\?v=b39c6d5e2a/);
