...
test('connected-product navigation links both shipped Android apps to their canonical Play Store listings', () => {
  ...
  assert.match(appHtml, /\/js\/app\.js\?v=2c1d21d360/);
});
...
test('all changed funnel assets use fresh cache keys', () => {
  ...
  assert.match(appHtml, /cloudApi\.js\?v=66/);
  ...
  assert.match(appHtml, /HomePage\.js\?v=62/);
  assert.match(appHtml, /app\.js\?v=2c1d21d360/);
