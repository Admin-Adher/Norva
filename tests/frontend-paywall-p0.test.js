...
test('connected-product navigation links both shipped Android apps to their canonical Play Store listings', () => {
  ...
  assert.match(appHtml, /\/js\/app\.js\?v=71baa0b2e4/);
});
...
test('all changed funnel assets use fresh cache keys', () => {
  ...
  assert.match(appHtml, /cloudApi\.js\?v=67/);
  assert.match(appHtml, /Settings\.js\?v=54/);
  ...
  assert.match(appHtml, /HomePage\.js\?v=64/);
  assert.match(appHtml, /app\.js\?v=71baa0b2e4/);
