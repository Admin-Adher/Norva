...
test('connected-product navigation links both shipped Android apps to their canonical Play Store listings', () => {
  ...
  assert.match(appHtml, /\/js\/app\.js\?v=54/);
});
...
test('all changed funnel assets use fresh cache keys', () => {
  ...
  assert.match(appHtml, /cloudApi\.js\?v=53/);
  assert.match(appHtml, /Settings\.js\?v=45/);
  ...
  assert.match(appHtml, /app\.js\?v=54/);
  assert.match(appJs, /AdminPage\.js\?v=88/);
