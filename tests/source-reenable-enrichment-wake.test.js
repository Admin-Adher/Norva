...
test('durable watchdog resumes both Xtream cursors and bounded M3U imports', () => {
  ...
  assert.match(m3uBranch, /if \(inM3uFinalize\)[\s\S]*runInBackground\(driveFinalizeToReady/);
  const providerBranch = m3uBranch.slice(m3uBranch.indexOf('// Discovery/provider-read recovery'));
  assert.ok(providerBranch.indexOf('claimM3uSyncLease(') < providerBranch.indexOf('runInBackground(syncCloudSource('));
  assert.ok(providerBranch.indexOf('if (!claim.claimed) continue') < providerBranch.indexOf('resumed.push('));
  assert.match(providerBranch, /m3uLease:\s*\{[\s\S]*token: leaseToken[\s\S]*attemptCount: claim\.attemptCount/);
