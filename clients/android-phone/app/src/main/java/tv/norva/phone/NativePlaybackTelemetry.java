...
final class NativePlaybackTelemetry {
    ...
    static void recordFirstFrame(final String authToken, final String sourceId,
                                 ...
                                 final long timeToFirstFrameMs, final boolean offline) {
        if (offline || authToken == null || authToken.isEmpty()
                || authToken.length() > 16_384 || itemType == null || itemType.isEmpty()
        ...
        Thread worker = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    out.write(payload);
                    out.close();
                    // Consume a small response so the connection can be released.
