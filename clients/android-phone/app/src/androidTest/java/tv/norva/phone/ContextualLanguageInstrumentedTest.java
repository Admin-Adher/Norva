        ...
        try {
            ...
            for(String locale:new String[]{"fr","ar"}) for(String kind:new String[]{"movies","series"}) {
                ...
                assertEquals("Scope survives both reset orders with keyboard for "+locale+"/"+kind, "\"ok\"",evaluate(instrumentation,holder.get(),
                    "(()=>{const f=contextFixture;f.audio.value='unidentified';f.source.value='fixture-source';f.categories.setSelected(['comedie']);f.source.value='';f.categories.setSelected([]);if(f.source.value||f.categories.getSelected().size)return 'source/category';f.categories.setSelected(['comedie']);f.source.value='fixture-source';f.categories.setSelected([]);f.source.value='';if(f.source.value||f.categories.getSelected().size)return 'category/source';if(f.audio.value!=='unidentified'||!f.audio.selectedOptions[0].text.startsWith(NorvaI18n.t('ui_web_audio_language_unidentified')))return 'unidentified filter lost';const sheet=document.getElementById('"+kind+"-filter-bar');if(sheet.inert||sheet.getAttribute('aria-hidden')!=='false')return 'inaccessible sheet';if(document.activeElement.id!=='"+kind+"-category-search')return 'focus lost';return 'ok';})()"));
