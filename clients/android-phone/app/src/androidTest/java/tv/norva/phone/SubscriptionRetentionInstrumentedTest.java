    ...
    private void verify(String language, int zoom) throws Exception {
        ...
        try {
            ...
            evaluate(i, holder.get(), "window.retentionResult='pending';(async()=>{try{"
                ...
                + "if(card.querySelector('button').getBoundingClientRect().bottom>innerHeight)throw Error('offer action below first screen');"
                + "if(Array.from(document.querySelectorAll('[data-retention-alternative]')).some(b=>b.getBoundingClientRect().height>0))throw Error('competing reactivation');"
                + "if(card.querySelector('details').open)throw Error('secondary terms initially expanded');"
                + "for(const s of ['.retention-price','.retention-duration','.retention-timing','.retention-renewal'])if(!card.querySelector(s)?.textContent.trim())throw Error('missing visible terms');"
