...
public class CatalogVersionTagsWebViewTest {
    ...
    private static String fixtureScript(String locale, String kind, int width) {
        return "(()=>{try{const api=NorvaI18n,M=MediaUtils;api.setPreference('"+locale+"');"
            ...
            + "const range=document.createRange();range.setStart(meta.firstChild,0);range.setEnd(meta.firstChild,prefix.length);if(range.getBoundingClientRect().width>meta.clientWidth+1)return 'provider hint clipped '+prefix;seen.add(meta.textContent);"
