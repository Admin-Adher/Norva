...
The resolver accepts a refreshed Plex token only when the persisted target, refreshed target, stable media key, external ID, full regional Plex part, source attribution and deterministic owned Selection source all agree. Regional versions of the two reviewed channel IDs are eligible; a refresh cannot change the selected regional part. Both targets must use the exact HTTPS Plex endpoint, canonical paths and exactly one `X-Plex-Token` parameter. Other URL changes and other Plex programmes fail closed. The complete refreshed URL is passed to the browser; nested playlist URLs retain their provider parameters.
