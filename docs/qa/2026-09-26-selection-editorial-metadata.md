...
## Flat catalogue root cause and owner artwork

All four owner-confirmed identities were applied to 128 visible owned titles across 32 ready Selection sources through the existing CAS writer, with per-row backups. Underworld rendered its French synopsis and a loaded 500px TMDB image in the filtered browser grid. The unfiltered grid exposed an additional defect: `attachOwnedMediaEditorialMetadata` treated every physical media generation as an unpublished projection, even when hydration returned a published global title. It also required that a multi-provider title's selected display generation belong to the queried source. Consequently, a successfully matched title could remain bare in flat searches.

The read correction distinguishes actual progressive payloads (P) from published titles (G). P still requires exact generation equality and its dedicated overlay. G uses the already validated exact owned visible media/variant association, source membership and current visibility-epoch hydration; only editorial fields are copied, preserving routing and per-file audio. Regression coverage retains foreign/stale/ambiguous and cross-generation P rejection and adds published same/different-source display-generation cases. 27 initial focused tests pass.

The owner also confirmed O Retorno as TMDB 1745971 and supplied a JPEG poster because that TMDB record has no image. The unchanged supplied file is stored as `public/img/posters/selection-o-retorno-1745971.jpg`; the Selection inventory uses a Norva URL. No synopsis is invented. Deployment and this artwork's runtime verification are pending.
