...
Verification: focused JVM classification tests plus four API 35 instrumentation tests, at font scale 1.3: ordinary H.264/AAC first frame, playable video mislabeled HTML, player controls, and HTML terminal state with one request, explicit retry with exactly one additional request, and Back. Final instrumentation result: **4 passed in 48.818 s**. Early emulator runs during boot were unstable; one process crashed and subsequent strict latency checks failed under system-wide boot broadcast contention. Waiting for the Android broadcast queue to become idle made the unchanged latency gate pass.

This native fix is separate from the installed Play version 35; publication has not yet been completed.
