...
- Latest local full Node suite: **3,721 passed, 0 failed, 8 skipped** (3,729 total), after the transactional email destination fix. The skips remain unproven runtime scenarios, not successes. Earlier generated locale validation, region model and JS/TS syntax checks are recorded in their release sections below.
...
- Last controlled USB-phone observation: Android Norva 1.3.17 (code 30), notification permission granted and Europe/Paris with `device` provenance. One authorized generic push was accepted by FCM and observed in the notification shade while Norva was backgrounded. **ADB currently detects no phone.** The native small-logo fix is on main but has not been installed or visually accepted on that phone. The user must perform the tap; lifecycle-specific deep links, foreground behavior, deduplication and conversion cancellation remain unproven.
