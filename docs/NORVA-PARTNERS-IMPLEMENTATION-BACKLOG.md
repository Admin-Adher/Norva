...
| `FIN-007` | Payouts | L | `PRT-007`, `DB-007`, `FIN-005` | `supabase/functions/norva-partners-payout/index.ts` ; `supabase/config.toml` | Adapter le rail de versement individuel : profil tokenisé, correspondance avec la personne KYC, batch mensuel, idempotence, confirmation, rejet et reprise. Aucun payout entreprise/KYB. `partners_payouts_live=false` bloque tout débit réel. |
| `FIN-008` | Payouts + Finance | M | `FIN-007` | même fonction ; RPC Admin | Générer un mode dry-run/export signé pour supervision humaine, seuil configuré par devise/politique, report, solde négatif et contre-écriture post-paiement. |
...
| `ADM-005` | Finance + Admin | L | `ADM-009`, `FIN-007`, `FIN-008` | mêmes chemins | Lots de versement : dry-run, validation humaine séparée, verrouillage, envoi, retour provider, retry, export comptable et rapprochement. Exiger `partners_payout_approve`; aucun bouton live si flag false. |
...
| `REL-004` | Finance | M | `REL-003`, `FIN-008`, `ADM-005` | payout dry-run/live | Premier lot mensuel supervisé : dry-run signé, échantillonnage manuel, envoi idempotent, rapprochement et rétro. |
