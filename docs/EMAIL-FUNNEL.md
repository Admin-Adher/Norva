...
| Payment confirmed | captured cross-rail ledger row → `cloud_billing_receipt_outbox` → leased lifecycle sender | authoritative paid Revolut or RevenueCat/store `INITIAL_PURCHASE`, `RENEWAL` or paid `NON_RENEWING_PURCHASE`; free trial/intro rows, internal accounts and active suppressions are excluded | env `RESEND_API_KEY` + DB `norva_cron_shared_secret` |
| Refund confirmed | immutable billing event → lifecycle intent → branded outbox | confirmed Revolut refund, or RevenueCat `CANCELLATION` with `cancel_reason=CUSTOMER_SUPPORT`; a store refund does not itself cancel access, so only a later `EXPIRATION` ends access | env `RESEND_API_KEY` |
