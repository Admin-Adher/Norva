...
| Gateway | **Pool résidentiel hybride** : `PROVIDER_PROXY_SOCKS_URLS` route les lanes Node en SOCKS5 rapide ; `PROVIDER_PROXY_URLS` garde le même slot HTTP pour ffmpeg/ffprobe. Les deux pools ont exactement la même forme et l'affinité reste **sticky par compte provider** (`host+username`), sans utiliser l'uid Norva. Une configuration partielle échoue fermée ; la santé expose uniquement la taille et le transport, jamais les endpoints. | `services/media-gateway` (GATEWAY_VERSION 136, `providerProxyTransport`, `providerProxyAffinityProtocol:1`), env Hetzner HTTP + SOCKS5 |
