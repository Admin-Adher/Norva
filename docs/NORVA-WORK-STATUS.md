...
| 4 | sous-titres | **OCR (Tesseract)** : sous-titres image (PGS/VOBSUB) → texte | 🟢 **MVP PGS livré** : pipeline gateway (`ocr_pgs.py` + `/ocr-async` + backoff 429) + edge v24 (`ocrEnqueue`) + **UI player on-demand** (bouton « OCR → text » par piste PGS), tous déployés ; parser + backend prouvés bout-en-bout. Reste : valider sur vrai flux PGS (le test a buté sur la limite connexion provider 429/401, pas le code) ; puis VOBSUB/DVB. Détail `docs/PHASE4-OCR-SUBTITLES.md` |
