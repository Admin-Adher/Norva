...
- **TMDB re-enrichment des titres à préfixe** (~138k titres `unmatched` matchant le regex de préfixe) :
  ...
  **par batches de 30k** (pour éviter le timeout MCP 60s), puis `norva_search_match_state` remis à
  ...
  `update cloud_titles set search_match_attempted_at=null where match_status='unmatched' and title ~ '^([A-Z]{2}|4K|8K|3D|2160P|1440P|1080P|720P|480P|360P|007)(-[A-Z0-9+]{1,6})*( [-–—▎▏▍▌│┃┆┊｜|] | -[A-Z0-9+]{1,6}- )';` puis `update norva_search_match_state set last_id=null, done=false where id=1;` (le head admet aussi les préfixes qualité digit-led "4K-AR - "/"8K - " du panel Strng IPTV 8K)
