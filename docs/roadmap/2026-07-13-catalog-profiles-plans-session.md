...
**Leçon** : pour la grille, le levier n'est PAS `cloud_titles` mais la couche brute — et le pipeline `norva_reconcile_catalog` existait déjà. Mon fix (2) a accéléré le côté `cloud_titles` (search/rails, réel) mais a **réinventé** l'étape `canonicalize` + créé une dette grille. Le fix (1) client v14 reste utile. **Décision finale : option B (nettoyage)** — `dedupe_cloud_titles_by_tmdb` **retiré** (migration de drop `20260714130000` + wiring `cronSearchMatch` reverté, commit ultérieur) ; on s'appuie sur le pipeline reconcile existant (merge + backfill) et son cron nocturne. La doc de référence complète est dans **`docs/architecture/catalog-dedup-and-grouping.md`**. Drain global accéléré à la main (boucle `norva_reconcile_catalog(NULL,5000)`).
