...
- Env optionnelle **`PROVIDER_PROXY_DEFAULT_NODE_TRANSPORT`** = `http` ou `socks5`. Elle choisit uniquement le transport de repli statique avant apprentissage (ou en mode shadow) sans retirer l'autre protocole des candidats adaptatifs. Valeur absente = comportement historique, SOCKS5 lorsqu'un pool SOCKS5 complet existe.
