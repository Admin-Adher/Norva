...
const { loadTranslationSelection } = require('./lib/selection');
...
function verifyLocalizations(options = {}) {
  const root = options.root || path.resolve(__dirname, '../..');
  const content = path.join(root, 'content/blog');
  const publicDir = path.join(root, 'public');
  const args = options.args || process.argv.slice(2);
  const preview = args.includes('--preview');
  const complete = args.includes('--expect-complete');
  ...
  const selection = loadTranslationSelection(content);
  const expectedVersions = selection.sources.length * LOCALES.length;
  const releasedSlugs = new Set(Object.keys(state).map(key => key.split('/')[1]));
  // During preparation, verify the existing released families without treating
  // unfinished additions as a release. Complete acceptance always covers every
  // explicitly selected source, including sources with no translation files yet.
  const sources = selection.sources.filter(source => preview || complete || releasedSlugs.has(source.source_slug)).map(source => source.filename);
  ...
  if (!preview) {
    try {
      ...
      // Validate every release hash, including releases outside the current selection.
  ...
  for (const locale of LOCALES) {
    ...
    for (const filename of sources) {
      ...
      try {
        ...
        if (locale.code !== 'en' && !preview && !state[key]) {
          if (complete) throw new Error('Selected translation is not released');
          continue;
        }
        ...
        if (!noindex) {
          const publishedAt = locale.code === 'en' ? englishState[String(Number(filename.match(/^(\d+)-/)[1]))] : state[key].published_at;
          const modifiedAt = locale.code === 'en' ? source.data.updated_at || publishedAt : state[key].updated_at || publishedAt;
          assert.ok(Number.isFinite(Date.parse(publishedAt)), 'valid publication date');
          assert.ok(Number.isFinite(Date.parse(modifiedAt)), 'valid modification date');
          assert.equal(article.datePublished, publishedAt, 'original publication date');
          assert.equal(article.dateModified, modifiedAt, 'reviewed modification date');
          const sitemapEntry = [...sitemap.matchAll(/<url>([\s\S]*?)<\/url>/g)].find(match => match[1].includes(`<loc>https://norva.tv${route}</loc>`));
          assert.equal(sitemapEntry?.[1].match(/<lastmod>([^<]+)<\/lastmod>/)?.[1], new Date(modifiedAt).toISOString().slice(0, 10), 'sitemap modification date');
        }
  ...
  if (complete && checked.length !== expectedVersions) findings.push({ error: `Expected ${expectedVersions} verified versions, got ${checked.length}` });
  let hubsChecked = 0;
  ...
  return { preview, complete, selectionTopics: selection.sources.length, expectedVersions, versionsChecked: checked.length, languageHubs: hubsChecked,
    existingEnglishArticles: Object.keys(JSON.parse(read(path.join(content, 'published-state.json'))).published).length,
    sitemapUrls: (sitemap.match(/<loc>/g) || []).length, checked, findings };
}

if (require.main === module) {
  try {
    const report = verifyLocalizations();
...
module.exports = { verifyLocalizations, verifyHubLayout };
