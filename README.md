<p align="center">
  <img src="public/img/logo-banner.png" alt="norva" height="60" />
</p>

# What is Norva?

Norva is a modern, web-based IPTV player featuring Live TV, EPG, Movies (VOD), and Series support. Built with performance and user experience in mind.
...
You can run Norva easily using Docker.
...
```env
OIDC_ISSUER_URL=https://your-idp.com/application/o/norva/
OIDC_CLIENT_ID=your_client_id
OIDC_CLIENT_SECRET=your_client_secret
OIDC_CALLBACK_URL=http://localhost:3000/api/auth/oidc/callback # Adjust for your Norva domain
```
...
Norva is a web-based application. By default, **video decoding is handled by your browser**. However, the built-in **smart transcoding system** automatically converts incompatible media (e.g., HEVC video, Dolby audio) into browser-friendly formats using FFmpeg.
...
Norva is optimized for **HLS (HTTP Live Streaming)**.
...
If you're running Norva behind a reverse proxy (Nginx, Caddy, Traefik) with HTTPS:
...
**Recommended Setup:**
1.  **Force Backend Proxy:** Enable this in **Settings → Transcoding → Network**. This routes middleware streams through Norva, bypassing CORS restrictions.
...
If you're using TVHeadend as your source, you may need to configure a few settings for streams to play correctly in Norva:

**Option 1: Enable Force Backend Proxy (Easiest)**
- In Norva, go to **Settings → Transcoding → Network**
...
- Add your Norva URL to **"CORS origin"** (e.g., `http://192.168.1.100:3000`)
...
If you are using `acestream-docker-home` or similar tools, it is **recommended** to use the HLS output format to reduce server load, though Norva can remux raw streams if needed.
